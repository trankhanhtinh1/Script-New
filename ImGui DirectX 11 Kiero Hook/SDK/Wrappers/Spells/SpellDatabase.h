...
namespace SDK {
    ...
    public:
        ...
        static bool Init(const std::string& jsonPath = "") {
            ...
            // Auto-detect path: look for Database.json next to DLL
            if (path.empty()) {
                path = GetDllDirectory() + "\\sdk\\Data\\Database.json";
            }
            ...
            // If file not found, still mark as initialized (empty database)
            s_initialized = true;
            return false;
        }
