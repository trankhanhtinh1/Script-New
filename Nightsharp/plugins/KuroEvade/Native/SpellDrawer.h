...
class SpellDrawer {
public:
    using VisualMap = std::unordered_map<const SDK::Skillshot*, SpellVisualInfo>;

    static void Draw(const std::vector<std::shared_ptr<SDK::Skillshot>>& skillshots,
        ...
        for (const auto& skillshot : skillshots) {
            ...
            DrawSkillshot(draw, *skillshot, info, style);
        }
    }
