...
struct Ziggs {
    static bool ProcessCast(const CastContext& context, ProcessResult& result) {
        ...
        spell2.Runtime.MissileSpeed = static_cast<int>(secondDistance * 1000.0f / 480.0f);
        spell3.Runtime.MissileSpeed = static_cast<int>(thirdDistance * 1000.0f / 430.0f);
        spell2.Runtime.Delay = static_cast<int>(context.Source.Runtime.Delay + firstDistance * 1000.0f /
            std::max(1.0f, static_cast<float>(context.Source.Runtime.MissileSpeed)));
        spell3.Runtime.Delay = static_cast<int>(spell2.Runtime.Delay + secondDistance * 1000.0f /
            std::max(1.0f, static_cast<float>(spell2.Runtime.MissileSpeed)));
