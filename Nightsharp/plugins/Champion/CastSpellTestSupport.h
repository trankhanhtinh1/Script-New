...
protected:
    ...
    uintptr_t ResolveNearestEnemyHero(
        ...
        const Vec3 playerPos =
            Globals::Read<Vec3>(player + Offset::All::Position);
    ...
    uintptr_t ResolveComboTarget(float range) const {
        ...
        const Vec3 playerPos = Globals::Read<Vec3>(
            ctx.localPlayer + Offset::All::Position);
        ...
        for (int i = 0; i < count; ++i) {
            ...
            const Vec3 position = Globals::Read<Vec3>(
                hero + Offset::All::Position);
