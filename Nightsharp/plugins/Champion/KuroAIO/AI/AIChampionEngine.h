...
inline constexpr int kManualInputLockMs = 165;
...
inline bool CanAct(bool reactive = false) {
    ...
    if (!reactive && Bool(HumanMenu, "RespectManual", true) &&
        LastManualSpellTick > 0 && now - LastManualSpellTick < kManualInputLockMs) {
        return false;
    }
        ...
        if (ValidEnemy(target, RuntimeSpells[index]->CurrentRange() + 100.0f)) {
            if (RuntimeSpells[index]->IsCharging()) {
                ...
                if (IsHardCrowdControlled(target) ||
                    static_cast<int>(prediction.Hitchance) >= static_cast<int>(spec.Hitchance)) {
...
inline void Game_OnUpdate(const SDK::Events::GameUpdateEventArgs&) {
    ...
    const int now = SDK::Variables::TickCount();
    const int decisionDelay = std::max(
        15,
        Slider(HumanMenu, "DecisionRate", 28));
...
inline void BuildMenu() {
    ...
    MenuRoot->Add(new MenuSeparator("Tactics", ActiveProfile->TacticalSummary));

    ComboMenu = MenuRoot->AddSubMenu(new Menu("Combo", "Pro combo coordinator"));
    BuildModeSpellToggles(ComboMenu, Mode::Combo, true);
    ComboMenu->Add(new MenuBool("FollowPlan", "Follow researched combo state machine", true));
    ComboMenu->Add(new MenuSlider(
        "MaxCommitEnemies", "Maximum enemies when committing mobility",
        ActiveProfile->MaximumCommitEnemies, 1, 5));
    ComboMenu->Add(new MenuBool(
        "AllowTurretDive", "Allow profile-approved lethal turret dive",
        ActiveProfile->AllowTurretDiveIfKillable));

    HarassMenu = MenuRoot->AddSubMenu(new Menu("Harass", "Short trade coordinator"));
    BuildModeSpellToggles(HarassMenu, Mode::Harass, false);
    HarassMenu->Add(new MenuSlider("Mana", "Minimum resource percent", 35, 0, 100));

    ClearMenu = MenuRoot->AddSubMenu(new Menu("Clear", "Lane, last-hit and jungle"));
    BuildModeSpellToggles(ClearMenu, Mode::LaneClear, false);
    ClearMenu->Add(new MenuSlider("Mana", "Minimum resource percent", 45, 0, 100));
    ClearMenu->Add(new MenuSlider("MinHits", "Minimum lane minions per AoE", 3, 1, 8));

    AutomaticMenu = MenuRoot->AddSubMenu(new Menu("Automatic", "Reactive intelligence"));
    BuildModeSpellToggles(AutomaticMenu, Mode::Automatic, false);
    AutomaticMenu->Add(new MenuBool("KillSecure", "Predicted lethal spell only", true));
    AutomaticMenu->Add(new MenuBool("Interrupt", "Interrupt dangerous channels", true));
    AutomaticMenu->Add(new MenuBool("AntiGapcloser", "Peel directed gapclosers", true));
    AutomaticMenu->Add(new MenuBool("EmergencyDefense", "Emergency heal, shield or disengage", true));
    AutomaticMenu->Add(new MenuBool("SaveAllies", "Protect pressured low-health allies", true));
    AutomaticMenu->Add(new MenuBool("GlobalExecute", "Allow automatic global execute", false));
    AutomaticMenu->Add(new MenuKeyBind(
        "ManualR", "Semi-manual smart ultimate", Keys::T, KeyBindType::Press));

    HumanMenu = MenuRoot->AddSubMenu(new Menu("Cooperation", "Player cooperation and timing"));
    HumanMenu->Add(new MenuBool("PreferSelected", "Prefer player's selected target", true));
    HumanMenu->Add(new MenuBool("RespectManual", "Yield briefly after manual spell input", true));
    HumanMenu->Add(new MenuBool("ManualResetsPlan", "Re-plan around manual spell casts", true));
    HumanMenu->Add(new MenuBool("PreserveAttacks", "Never cancel a valuable attack windup", true));
    HumanMenu->Add(new MenuSlider(
        "Humanizer", "Minimum delay between assisted casts (ms)",
        ActiveProfile->BaseHumanizerMs, 20, 220));
    HumanMenu->Add(new MenuSlider(
        "DecisionRate", "Decision refresh interval (ms)", 28, 10, 120));

    DrawMenu = MenuRoot->AddSubMenu(new Menu("Draw", "Visual coaching"));
    for (int index = 0; index < 4; ++index) {
        char key[16] = {};
        char label[48] = {};
        _snprintf_s(key, sizeof(key), _TRUNCATE, "Draw%s", SlotName(index));
    ...
    DrawMenu->Add(new MenuBool("DrawTarget", "Highlight coordinated target", true));
    DrawMenu->Add(new MenuBool("DrawPlan", "Show current combo plan step", true));
