...
inline bool CastW(const AIHeroClient& target, Mode mode, bool reactive = false) {
    ...
    if (!FinitePoint(endpoint) || player.Position().Distance2D(endpoint) > range ||
        SDK::NavMesh::IsWall(endpoint) || Engine::UnderEnemyTurret(endpoint) ||
        ControllerHelpers::ProjectileWallBlocksFromPlayer(endpoint, 60.0f)) return false;
...
inline constexpr ChampionController Controller = [] {
    ...
    controller.ChampionName = "TahmKench";
