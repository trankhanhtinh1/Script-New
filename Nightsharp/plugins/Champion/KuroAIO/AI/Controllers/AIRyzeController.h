...
inline std::array<FluxRecord, 96> FluxRecords = {};
...
inline constexpr int kManualOwnershipMs = 430;
inline constexpr int kManualResetWindowMs = 680;
inline constexpr int kCombatSequenceMs = 5200;
inline constexpr int kWaveSequenceMs = 7600;
...
inline constexpr std::array<MobilityRule, 39> MobilityRules = {
    ...
    MobilityRule{ "Rengar", SDK::SpellSlot::R },
...
inline void BuildMenu(Menu* root) {
    ...
    TacticsMenu->Add(new MenuBool(
        "KillSecure", "Secure only with mitigated spell damage", true));
    TacticsMenu->Add(new MenuSlider(
        "KiteHp", "Prefer two-rune escape below health (%)", 36, 5, 80));
    TacticsMenu->Add(new MenuSlider(
        "ManualOwnershipMs", "Yield after player spell input (ms)",
        kManualOwnershipMs, 150, 1000));
    TacticsMenu->Add(new MenuSeparator(
        "Ownership",
        "Movement, target selection, attacks, Flash and every Realm Warp entry remain player-owned."));
    ...
    ComboMenu->Add(new MenuBool(
        "FullDps", "Use Q-E-Q-W-Q-E-Q only on committed safe targets", true));
    ComboMenu->Add(new MenuBool(
        "ReserveEWQ", "Reserve E-W-Q mana outside all-in windows", true));
    ComboMenu->Add(new MenuBool(
        "PreserveSpeedQ", "Preserve two-rune Q in nonlethal harass", true));
    ComboMenu->Add(new MenuBool(
        "WeaveAutos", "Leave safe cooldown gaps to Orbwalker attacks", true));
    ComboMenu->Add(new MenuBool(
        "AssistManualReset", "Buffer a clean Q after player W/E", true));
    ComboMenu->Add(new MenuSlider(
        "HarassMana", "Minimum mana for Q-E-Q harass (%)", 47, 0, 100));
    ComboMenu->Add(new MenuSeparator(
        "ResetTruth",
        "W and E reset Overload; E-W-Q and W-Q-E-Q remain available while Q is cooling down."));
    ...
    FluxMenu->Add(new MenuBool(
        "IndirectEQ", "Use a minion E-Q bridge onto a champion", true));
    FluxMenu->Add(new MenuSeparator(
        "FirstBody",
        "Every Q resolves moving champion, minion and monster interception before it is cast."));
    ...
    PrisonMenu->Add(new MenuBool(
        "Interrupt", "E-W root interruptible channels", true));
    PrisonMenu->Add(new MenuBool(
        "AntiGapcloser", "E-W-Q committed dash endpoints", true));
    PrisonMenu->Add(new MenuBool(
        "ProtectCarry", "Root the diver threatening the highest-value ally", true));
    PrisonMenu->Add(new MenuSlider(
        "PeelDistance", "Maximum diver distance from protected ally", 525, 250, 750));
    PrisonMenu->Add(new MenuSeparator(
        "NoFakeRoot",
        "Unfluxed W is treated as a slow; only Flux-W is allowed to satisfy an interrupt root."));
    ...
    WaveMenu->Add(new MenuBool(
        "Lane", "Use champion-specific Flux wave logic", true));
    WaveMenu->Add(new MenuBool(
        "Jungle", "Use reset branches against jungle monsters", true));
    WaveMenu->Add(new MenuSlider(
        "MinimumQHits", "Minimum victims for an existing-Flux Q", 3, 1, 7));
    WaveMenu->Add(new MenuSlider(
        "MinimumEEQHits", "Minimum victims before starting E-E-Q", 4, 2, 8));
    WaveMenu->Add(new MenuSlider(
        "LaneMana", "Minimum E-E-Q lane mana (%)", 42, 0, 100));
    WaveMenu->Add(new MenuSlider(
        "LastHitMana", "Minimum exact-Q last-hit mana (%)", 28, 0, 100));
    WaveMenu->Add(new MenuSlider(
        "JungleMana", "Minimum jungle mana (%)", 24, 0, 100));
    WaveMenu->Add(new MenuBool(
        "HoldForChampion", "Hold wave spells while an enemy contests", true));
    WaveMenu->Add(new MenuSlider(
        "ChampionHoldRange", "Champion contest range", 1050, 500, 1600));
    WaveMenu->Add(new MenuSeparator(
        "WaveSurvival",
        "The chosen second-E and Q detonation body must survive the first E."));
    ...
    WarpMenu->Add(new MenuBool(
        "EmergencyOptIn", "Allow lethal-health cursor escape R", false));
    WarpMenu->Add(new MenuSlider(
        "EmergencyHp", "Emergency R health threshold (%)", 17, 5, 50));
    WarpMenu->Add(new MenuBool(
        "UnsafeManual", "Allow manual turret/blind endpoint override", false));
    WarpMenu->Add(new MenuSeparator(
        "NoAbduction",
        "R rejects walls, losing arrivals, likely interrupts and allied protected channels inside the portal."));
    ...
    CoachMenu->Add(new MenuBool(
        "DrawQ", "Draw planned Q line and actual first body", true));
    CoachMenu->Add(new MenuBool(
        "DrawFlux", "Draw confirmed/predicted Flux expiry", true));
    CoachMenu->Add(new MenuBool(
        "DrawProtected", "Mark the dynamically protected ally", true));
    CoachMenu->Add(new MenuBool(
        "DrawWarp", "Draw the last verified Realm Warp endpoint", true));
    CoachMenu->Add(new MenuBool(
        "DrawState", "Show posture, branch, runes and ownership", true));
}
...
inline constexpr const char* Scenarios[] = {
    ...
    "Draw the last verified Realm Warp endpoint",
