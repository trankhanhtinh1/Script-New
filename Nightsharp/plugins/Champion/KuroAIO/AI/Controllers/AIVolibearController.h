...
inline bool CastE(const AIHeroClient& target, Mode mode, bool reactive = false,
    ...
    if (hasTarget && (ProjectileWallBlocksFromPlayer(center, kQWidth) ||
        !ProjectileCollision(player.Position(), center, target.Position(), kQWidth,
...
inline bool SafeREndpoint(const Vector3& endpoint, const AIHeroClient& target,
    ...
    if (!RReachable(player.Position(), endpoint, target.BoundingRadius()) ||
        ProjectileWallBlocksFromPlayer(endpoint, kRRadius)) return false;
...
inline constexpr ChampionController Controller = [] {
    ...
    controller.ChampionName = "Volibear";
