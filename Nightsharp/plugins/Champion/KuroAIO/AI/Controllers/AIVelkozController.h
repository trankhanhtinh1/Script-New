...
inline bool SafeRayPosition(const Vector3& endpoint, bool lethal) {
    const auto player = GameObjects::Player();
    if (!player.IsValid() || !RWallSafe(player.Position(), endpoint,
        ControllerHelpers::ProjectileWallBlocksFromPlayer(endpoint, kRWidth * 0.5f)))
        return false;
...
inline constexpr ChampionController Controller = [] {
    ...
    controller.ChampionName = "Velkoz";
