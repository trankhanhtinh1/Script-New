...
inline bool EnemyCanCleanse(const AIHeroClient& target) {
    if (!target.IsValid()) return false;
    if (SDK::CanUseItem(target, 3140) || SDK::CanUseItem(target, 3139) ||
        SDK::CanUseItem(target, 6035)) {
        return true;
    }
    return (ChampionIs(target, "Olaf") &&
            EnemySpellReady(target, SDK::SpellSlot::R)) ||
           (ChampionIs(target, "Gangplank") &&
            EnemySpellReady(target, SDK::SpellSlot::W)) ||
           (ChampionIs(target, "Alistar") &&
            EnemySpellReady(target, SDK::SpellSlot::R)) ||
...
inline void BuildMenu(Menu* root) {
    ...
    RoleMenu->Add(new MenuBool(
        "RespectCursor", "Require cursor agreement before proactive Q", true));
    RoleMenu->Add(new MenuBool(
        "PeelBeforeEngage", "Protect a pressured carry before engaging", true));
    RoleMenu->Add(new MenuSlider(
        "HarassFollowup", "Minimum allies able to follow harass Q", 1, 0, 3));
    RoleMenu->Add(new MenuSeparator(
        "Ownership",
        "Movement, attack-move, Hold, Stop, Flash and Smite stay player-owned."));
    ...
    BandageMenu->Add(new MenuBool(
        "ReserveSecond", "Reserve one Q for Flash, dash or peel", true));
    ...
    BandageMenu->Add(new MenuSlider(
        "MaximumNakedRange", "Maximum ordinary naked-Q center range",
        1000, 700, 1160));
    BandageMenu->Add(new MenuSlider(
        "MaxArrivalEnemies", "Maximum enemies around Q arrival", 3, 1, 5));
    BandageMenu->Add(new MenuSlider(
        "MinimumSafety", "Minimum Q-arrival safety score", -180, -1200, 600));
    BandageMenu->Add(new MenuBool(
        "RespectDashHazards", "Reject ready Poppy/Taliyah/Cassiopeia denial", true));
    BandageMenu->Add(new MenuBool(
        "ShieldMobility", "Allow shielded Q only as supported R mobility", false));
    BandageMenu->Add(new MenuBool(
        "Bridge", "Use minion/monster Q bridge with two charges", true));
    BandageMenu->Add(new MenuSlider(
        "BridgeMinimumGain", "Minimum reach gained by Q bridge", 300, 150, 650));
    BandageMenu->Add(new MenuBool(
        "LayerCrowdControl", "Delay Q2 until current CC is nearly ending", true));
    BandageMenu->Add(new MenuSlider(
        "OverlapAllowance", "Allowed Q2 CC overlap (ms)", 100, 0, 300));
    BandageMenu->Add(new MenuBool(
        "SuppressQAutoForR", "Cancel Q's forced AA only for immediate R", true));
    BandageMenu->Add(new MenuBool(
        "PeelQ", "Q a diver threatening the protected ally", true));
    BandageMenu->Add(new MenuBool(
        "InterruptQ", "Interrupt a reachable channel with Q", true));
    BandageMenu->Add(new MenuBool(
        "EscapeAnchor", "Q a cursor-aligned safe unit while fleeing", true));
    BandageMenu->Add(new MenuBool(
        "HarassQ", "Use Q harass only with two charges and follow-up", true));
    BandageMenu->Add(new MenuSlider(
    ...
    DespairMenu->Add(new MenuBool(
        "AutoToggle", "Automatically manage W around real contact", true));
    DespairMenu->Add(new MenuBool(
        "PreToggleQ", "Enable W shortly before a confirmed Q arrival", true));
    DespairMenu->Add(new MenuSlider(
        "PreToggleLead", "Maximum pre-arrival W lead (ms)", 650, 150, 950));
    DespairMenu->Add(new MenuSlider(
        "FlatReserve", "Minimum mana reserve before W drain", 70, 0, 300));
    DespairMenu->Add(new MenuSlider(
        "OffDelay", "No-contact delay before W turns off (ms)", 420, 100, 1200));
    DespairMenu->Add(new MenuBool(
        "KeepEpic", "Keep W on while an epic monster is in range", true));
    ...
    TantrumMenu->Add(new MenuBool(
        "WeaveCurse", "Wait for an in-range AA before ordinary champion E", true));
    TantrumMenu->Add(new MenuBool(
        "WaitAttackRefund", "Wait briefly when incoming autos refresh E", true));
    TantrumMenu->Add(new MenuSlider(
        "RefundWaitMs", "Maximum E-refund wait (ms)", 480, 100, 900));
    TantrumMenu->Add(new MenuSeparator(
        "RefundRule", "Each basic attack that hits removes exactly 0.75s from E."));
    ...
    UltimateMenu->Add(new MenuBool(
        "MultiTarget", "R a valuable multi-target cluster", true));
    ...
    UltimateMenu->Add(new MenuBool(
        "AvoidCCOverlap", "Penalize R over long existing hard CC", true));
    UltimateMenu->Add(new MenuBool(
        "RespectCleanse", "Discount ready QSS and champion cleanses", true));
    UltimateMenu->Add(new MenuBool(
        "BufferAfterQ", "Queue R during Q dash when arrival demands it", true));
    UltimateMenu->Add(new MenuSlider(
        "BufferLead", "Maximum Q-arrival R buffer lead (ms)", 520, 120, 900));
    UltimateMenu->Add(new MenuBool(
        "SingleCarryArrival", "Immediate R one Flash-ready high-value carry", true));
    UltimateMenu->Add(new MenuSlider(
        "SingleCarryHp", "HP ceiling for single-carry arrival R (%)", 42, 10, 90));
    UltimateMenu->Add(new MenuBool(
        "HighValuePick", "Use supported R for a high-value current target", true));
    UltimateMenu->Add(new MenuSlider(
        "PickHp", "HP ceiling for supported carry pick (%)", 58, 15, 95));
    UltimateMenu->Add(new MenuBool(
        "LethalSingle", "R one target only when R is uniquely lethal", true));
    UltimateMenu->Add(new MenuBool(
        "PeelR", "R a critical threat on the protected ally", true));
    UltimateMenu->Add(new MenuSlider(
        "PeelAllyHp", "Protected ally HP for critical peel (%)", 42, 10, 85));
    UltimateMenu->Add(new MenuBool(
        "AntiDashR", "Knock down a committed dash with R", true));
    UltimateMenu->Add(new MenuBool(
        "InterruptR", "Use R when Q cannot interrupt in time", true));
    UltimateMenu->Add(new MenuBool(
        "SurvivalR", "R nearby enemies under lethal incoming pressure", true));
    UltimateMenu->Add(new MenuSlider(
        "SurvivalHp", "Player HP for survival R (%)", 27, 5, 65));
    UltimateMenu->Add(new MenuBool(
        "AfterManualFlash", "Cast smart R after the player manually Flashes", true));
    UltimateMenu->Add(new MenuBool(
        "FleeR", "Use R while fleeing from multiple pursuers", true));
    UltimateMenu->Add(new MenuSlider(
        "FleeTargets", "Minimum flee-R targets", 2, 1, 5));
    UltimateMenu->Add(new MenuSeparator(
        "RFlash",
        "Manual R->Flash is coached only; the controller never presses Flash."));

    FarmMenu = TacticsMenu->AddSubMenu(new Menu(
        "Farm", "Jungle E-refund and conservative lane clear"));
    FarmMenu->Add(new MenuBool(
        "JungleAbilities", "Use champion abilities in jungle", true));
    FarmMenu->Add(new MenuBool(
        "JungleQ", "Q a distant jungle target without losing combat reserve", true));
    FarmMenu->Add(new MenuBool(
        "JungleE", "Use E on jungle contact", true));
    FarmMenu->Add(new MenuSlider(
        "JungleEMinimum", "Minimum jungle units for E", 1, 1, 6));
    FarmMenu->Add(new MenuBool(
        "LaneE", "Use E for configured lane clear/last hits", true));
    FarmMenu->Add(new MenuBool(
    ...
    CoachMenu->Add(new MenuBool(
        "DrawQ", "Draw Q range, first collision and arrival safety", true));
    CoachMenu->Add(new MenuBool(
        "DrawAura", "Draw W and E effect radii", false));
    CoachMenu->Add(new MenuBool(
        "DrawR", "Draw R radius, hit set and primary target", true));
    CoachMenu->Add(new MenuBool(
        "DrawPeel", "Draw protected ally and current diver", true));
    CoachMenu->Add(new MenuBool(
        "DrawState", "Draw posture, sequence, ammo, W/E/R state", true));
    CoachMenu->Add(new MenuBool(
        "DrawRFlashWindow", "Coach the short manual R->Flash window", true));
}
