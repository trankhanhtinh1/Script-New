...
inline void BuildMenu(Menu* root) {
    ...
    TacticsMenu->Add(new MenuBool(
        "KillSecure", "Secure only with conservative mitigated damage", true));
    ...
    TacticsMenu->Add(new MenuSeparator(
        "Ownership",
        "Movement, attacks, Flash, R ride/recast and macro destination remain player-owned."));

    QMenu = TacticsMenu->AddSubMenu(new Menu(
        "ThreadedVolley", "Accelerating volley, boulder and Worked Ground"));
    QMenu->Add(new MenuList(
        "QHitchance", "Ordinary moving-target Q prediction",
        { "Medium", "High", "Very high", "Immobile only" }, 2));
    QMenu->Add(new MenuBool(
        "AoeBridge", "Use first-body minion/monster AoE to hit champions", true));
    QMenu->Add(new MenuBool(
        "ManageGround", "Preserve useful Worked Ground layout", true));
    QMenu->Add(new MenuSeparator(
        "ProjectileTruth",
        "Volley Q accelerates/decelerates at live speed; Big Q is a slower fixed 2000 projectile."));
    ...
    ComboMenu->Add(new MenuBool(
        "AssistManualBoulder", "Continue a clean player Big Q with W-E", true));
    ComboMenu->Add(new MenuBool(
        "AssistManualW", "Place E after a clean player W", true));
    ComboMenu->Add(new MenuBool(
        "AssistManualE", "Find the correct W vector after player E", true));
    ComboMenu->Add(new MenuSlider(
        "HarassMana", "Minimum mana for committed harass (%)", 48, 0, 100));
    ComboMenu->Add(new MenuSeparator(
        "OrderTruth",
        "W-E is faster but may miss late rows; E-W is slower and preferred into close/dashing targets."));
    ...
    MineMenu->Add(new MenuSeparator(
        "LiveRows",
        "Track 22 mines: 2 in row one, then 4 in each of five rows, 0.17 seconds apart."));
    MineMenu->Add(new MenuSeparator(
        "Falloff",
        "Detonation scoring uses 100/75/50/25 percent and never exceeds the live 2.5 multiplier."));

    PeelMenu = TacticsMenu->AddSubMenu(new Menu(
        "Peel", "Anti-dash, interrupt and protected-carry vectors"));
    PeelMenu->Add(new MenuBool(
        "AntiGapcloser", "E the committed path, then W away", true));
    PeelMenu->Add(new MenuBool(
        "Interrupt", "Use reliable W on channels with enough time", true));
    PeelMenu->Add(new MenuBool(
        "ProtectCarry", "Reserve W/E for the highest-value allied carry", true));
    PeelMenu->Add(new MenuSlider(
        "PeelDistance", "Maximum threat distance from protected ally", 560, 280, 820));
    PeelMenu->Add(new MenuSeparator(
        "NoDelivery",
        "Never shove a diver closer to the carry unless that exact vector increases separation."));
    ...
    FarmMenu->Add(new MenuBool(
        "Lane", "Use champion-specific Q/E wave logic", true));
    FarmMenu->Add(new MenuBool(
        "UseE", "Use E only on a valuable wave envelope", true));
    FarmMenu->Add(new MenuSlider(
        "MinimumQHits", "Minimum first-impact Q AoE victims", 3, 1, 7));
    FarmMenu->Add(new MenuSlider(
        "MinimumEHits", "Minimum E initial wave hits", 4, 2, 10));
    FarmMenu->Add(new MenuSlider(
        "LaneMana", "Minimum lane-clear mana (%)", 36, 0, 100));
    FarmMenu->Add(new MenuBool(
        "Jungle", "Use current Taliyah jungle logic", true));
    FarmMenu->Add(new MenuBool(
        "JungleE", "Use live 225-percent E monster modifier", true));
    FarmMenu->Add(new MenuBool(
        "PreserveVolley", "Do not replace full-volley camp DPS with Big Q", true));
    FarmMenu->Add(new MenuSlider(
        "JungleMana", "Minimum jungle mana (%)", 22, 0, 100));
    FarmMenu->Add(new MenuBool(
        "HoldEForChampion", "Reserve E while an enemy contests farm", true));
    FarmMenu->Add(new MenuSlider(
    ...
    WallMenu->Add(new MenuSlider(
        "EscapeHp", "Classify manual wall as escape below HP (%)", 24, 5, 70));
    WallMenu->Add(new MenuBool(
        "UnsafeManual", "Allow explicit endpoint override (still protect channels)", false));
    WallMenu->Add(new MenuSeparator(
        "NoRide",
        "The controller casts only the first wall after G; ride, jump-off and early destroy remain player-owned."));
    ...
    CoachMenu->Add(new MenuBool(
        "DrawGround", "Draw every active Worked Ground and expiry", true));
    CoachMenu->Add(new MenuBool(
        "DrawMines", "Draw spawned versus pending E rows", true));
    CoachMenu->Add(new MenuBool(
        "DrawQ", "Draw planned Q and true first-impact AoE", true));
    CoachMenu->Add(new MenuBool(
        "DrawW", "Draw W center, vector and 400-unit destination", true));
    CoachMenu->Add(new MenuBool(
        "DrawProtected", "Mark the dynamically protected ally", true));
    CoachMenu->Add(new MenuBool(
        "DrawWall", "Draw last verified manual wall", true));
    CoachMenu->Add(new MenuBool(
        "DrawState", "Show posture, sequence, Q form and ownership", true));
}
