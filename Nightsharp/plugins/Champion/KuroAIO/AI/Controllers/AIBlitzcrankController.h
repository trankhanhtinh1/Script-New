...
inline bool ChampionNameIn(const AIHeroClient& champion,
                           const std::array<const char*, N>& names) {
    if (!champion.IsValid()) return false;
    for (const char* name : names) {
        if (ControllerHelpers::NameEquals(
                champion.CharacterName().c_str(), name)) return true;
    }
    return false;
}

inline PullArchetype ClassifyPullArchetype(const AIHeroClient& target) {
    static constexpr std::array<const char*, 28> carries = {
        "Aphelios", "Ashe", "Caitlyn", "Corki", "Draven", "Ezreal",
        "Jhin", "Jinx", "KaiSa", "Kalista", "KogMaw", "Lucian",
        "MissFortune", "Nilah", "Quinn", "Samira", "Senna", "Sivir",
        "Smolder", "Tristana", "Twitch", "Varus", "Vayne", "Xayah",
        "Zeri", "Azir", "Kayle", "Ryze",
    };
    static constexpr std::array<const char*, 12> enchanters = {
        "Janna", "Karma", "Lulu", "Milio", "Nami", "Renata",
        "RenataGlasc", "Seraphine", "Sona", "Soraka", "Yuumi", "Ivern",
    };
    static constexpr std::array<const char*, 13> artillery = {
        "Hwei", "Jayce", "JayceHammer", "JayceStanceHtG", "Lux",
        "Nidalee", "NidaleeCougar", "Velkoz", "Xerath", "Ziggs",
        "Zoe", "Karthus", "Heimerdinger",
    };
    static constexpr std::array<const char*, 18> assassins = {
        "Akali", "Evelynn", "Fizz", "Kassadin", "Katarina", "Khazix",
        "Leblanc", "Naafiri", "Nocturne", "Qiyana", "Rengar", "Shaco",
        "Talon", "Zed", "Ekko", "Diana", "Kayn", "BlueKayn",
    };
    static constexpr std::array<const char*, 25> divers = {
        "Ambessa", "Belveth", "Briar", "Camille", "Diana", "Elise",
        "Hecarim", "Irelia", "JarvanIV", "Jax", "Kled", "LeeSin",
        "MasterYi", "Nocturne", "Olaf", "Pantheon", "RekSai",
        "Renekton", "Rengar", "Sylas", "Vi", "Viego", "Warwick",
        "MonkeyKing", "XinZhao",
    };
    static constexpr std::array<const char*, 16> juggernauts = {
        "Aatrox", "Darius", "DrMundo", "Garen", "Illaoi", "Mordekaiser",
        "Nasus", "Sett", "Shyvana", "Skarner", "Trundle", "Udyr",
        "Urgot", "Volibear", "Yorick", "KSante",
    };
    static constexpr std::array<const char*, 19> engageBombs = {
        "Alistar", "Amumu", "Galio", "Gragas", "Kennen", "Leona",
        "Malphite", "Maokai", "Neeko", "Nautilus", "Nunu",
        "NunuWillump", "Ornn", "Rakan", "Rell", "Sejuani", "Sion",
        "Zac", "Fiddlesticks",
    };
    static constexpr std::array<const char*, 10> wardens = {
        "Braum", "Poppy", "Shen", "TahmKench", "Taric", "Thresh",
        "Rammus", "ChoGath", "Blitzcrank", "KSante",
    };
    if (ChampionNameIn(target, carries)) return PullArchetype::Carry;
    if (ChampionNameIn(target, enchanters)) return PullArchetype::Enchanter;
    if (ChampionNameIn(target, artillery)) return PullArchetype::Artillery;
    if (ChampionNameIn(target, assassins)) return PullArchetype::Assassin;
    if (ChampionNameIn(target, divers)) return PullArchetype::Diver;
    if (ChampionNameIn(target, juggernauts)) return PullArchetype::Juggernaut;
    if (ChampionNameIn(target, engageBombs)) return PullArchetype::EngageBomb;
    if (ChampionNameIn(target, wardens)) return PullArchetype::Warden;
...
inline constexpr std::array<MobilityRule, 74> MobilityRules = {
    ...
    MobilityRule{ "Akali", SDK::SpellSlot::R },
    ...
    MobilityRule{ "Elise", SDK::SpellSlot::E },
    MobilityRule{ "Ezreal", SDK::SpellSlot::E },
    MobilityRule{ "Fiora", SDK::SpellSlot::Q },
    MobilityRule{ "Fizz", SDK::SpellSlot::Q },
    ...
    MobilityRule{ "Gwen", SDK::SpellSlot::E },
    ...
    MobilityRule{ "Kayn", SDK::SpellSlot::E },
    MobilityRule{ "Kayn", SDK::SpellSlot::R },
    MobilityRule{ "Khazix", SDK::SpellSlot::E },
    MobilityRule{ "Kindred", SDK::SpellSlot::Q },
    ...
    MobilityRule{ "Malphite", SDK::SpellSlot::R },
    MobilityRule{ "MasterYi", SDK::SpellSlot::Q },
    MobilityRule{ "Naafiri", SDK::SpellSlot::E },
    MobilityRule{ "Nidalee", SDK::SpellSlot::W },
    MobilityRule{ "Nilah", SDK::SpellSlot::E },
    MobilityRule{ "Nocturne", SDK::SpellSlot::R },
    MobilityRule{ "Pantheon", SDK::SpellSlot::W },
    MobilityRule{ "Pyke", SDK::SpellSlot::E },
    MobilityRule{ "Qiyana", SDK::SpellSlot::E },
    MobilityRule{ "Qiyana", SDK::SpellSlot::W },
    MobilityRule{ "Quinn", SDK::SpellSlot::E },
    ...
    MobilityRule{ "Riven", SDK::SpellSlot::Q },
    MobilityRule{ "Riven", SDK::SpellSlot::E },
    MobilityRule{ "Samira", SDK::SpellSlot::E },
    MobilityRule{ "Sejuani", SDK::SpellSlot::Q },
    MobilityRule{ "Shaco", SDK::SpellSlot::Q },
    MobilityRule{ "Shen", SDK::SpellSlot::E },
    MobilityRule{ "Shyvana", SDK::SpellSlot::R },
    ...
    MobilityRule{ "Vi", SDK::SpellSlot::R },
    MobilityRule{ "Viego", SDK::SpellSlot::W },
    MobilityRule{ "Viego", SDK::SpellSlot::R },
    MobilityRule{ "Warwick", SDK::SpellSlot::Q },
    MobilityRule{ "Warwick", SDK::SpellSlot::R },
    MobilityRule{ "MonkeyKing", SDK::SpellSlot::E },
    MobilityRule{ "XinZhao", SDK::SpellSlot::E },
    MobilityRule{ "Yasuo", SDK::SpellSlot::E },
    MobilityRule{ "Yone", SDK::SpellSlot::E },
    MobilityRule{ "Yone", SDK::SpellSlot::R },
    MobilityRule{ "Zac", SDK::SpellSlot::E },
...
inline SDK::HitChance RequiredQHitchance(HookPurpose purpose,
    ...
    case 0: return SDK::HitChance::Medium;
    case 1: return SDK::HitChance::High;
    case 3: return SDK::HitChance::Immobile;
    default: return SDK::HitChance::VeryHigh;
    }
}
...
inline void BuildMenu(Menu* root) {
    ...
    TacticsMenu->Add(new MenuBool(
        "KillSecure", "Secure only with exact mitigated damage", true));
    TacticsMenu->Add(new MenuSlider(
        "EmergencyHp", "Emergency survival health threshold (%)",
        24, 5, 70));
    TacticsMenu->Add(new MenuSlider(
        "ManualOwnershipMs", "Yield after player spell input (ms)",
        kManualOwnershipMs, 150, 1000));
    TacticsMenu->Add(new MenuSeparator(
        "PlayerOwnership",
        "Movement, attacks, attack-move, warding, Flash and Hexflash remain player-owned."));

    RoleMenu = TacticsMenu->AddSubMenu(new Menu(
        "Posture", "Catch, front-to-back and carry protection"));
    RoleMenu->Add(new MenuBool(
        "ProtectCarry", "Peel a threatened carry before looking for catches", true));
    RoleMenu->Add(new MenuSlider(
        "PeelThreatScore", "Minimum protected-carry threat score",
        590, 250, 1200));
    RoleMenu->Add(new MenuBool(
        "ReservePeelMana", "Keep enough mana for Power Fist", true));
    ...
    QMenu->Add(new MenuSlider(
        "MinimumFollowup", "Allies required at pull landing", 1, 0, 4));
    QMenu->Add(new MenuSlider(
        "HarassMana", "Minimum harass mana (%)", 58, 0, 100));
    QMenu->Add(new MenuBool(
        "HoldForWE", "Hold Q when safe W-E makes it guaranteed", true));
    QMenu->Add(new MenuBool(
        "Peel", "Use Q as protected-carry displacement", true));
    QMenu->Add(new MenuBool(
        "Interrupt", "Interrupt channels outside Power Fist range", true));
    QMenu->Add(new MenuBool(
        "AntiGapcloser", "Hook committed dash endpoints", true));
    QMenu->Add(new MenuBool(
        "ObjectiveJungler", "Pull visible Smite holder from epic objective", true));
    QMenu->Add(new MenuBool(
        "StasisExit", "Time Q arrival to enemy stasis exit", true));
    QMenu->Add(new MenuSlider(
        "StasisEarlyMs", "Stasis-exit latency bias (ms)", 45, 0, 140));
    QMenu->Add(new MenuBool(
        "Flee", "Displace nearest pursuer during flee", true));
    QMenu->Add(new MenuSeparator(
        "NoGrief",
        "Healthy assassins, divers, juggernauts and engage tanks are not delivered onto the protected carry."));

    WMenu = TacticsMenu->AddSubMenu(new Menu(
        "Overdrive", "Walk-up pressure and delayed self-slow"));
    WMenu->Add(new MenuBool(
        "WalkUpE", "Use W only when it reaches a safe guaranteed E", true));
    WMenu->Add(new MenuBool(
        "AfterHook", "Use W after hook for contact and attack speed", true));
    WMenu->Add(new MenuBool(
        "HookAngle", "Use W when player path creates a better Q angle", true));
    WMenu->Add(new MenuBool(
        "Peel", "Use W to reach an urgent peel target", true));
    WMenu->Add(new MenuBool(
        "Flee", "Use W on explicit flee route", true));
    WMenu->Add(new MenuBool(
        "HarassWalkUp", "Allow mana-expensive W-E harass", false));
    WMenu->Add(new MenuBool(
        "Roam", "Use W on long uncontested player-led roam paths", false));
    WMenu->Add(new MenuSeparator(
        "SelfSlow",
        "The five-second speed decay and following 1.5-second 30% self-slow are included in every W decision."));

    EMenu = TacticsMenu->AddSubMenu(new Menu(
        "PowerFist", "Exact target, pre-arm and AA reset timing"));
    EMenu->Add(new MenuBool(
        "AAReset", "Use AA-E-AA only when target cannot escape", true));
    EMenu->Add(new MenuBool(
        "PreArmHook", "Arm E during Q flight against escape-ready targets", true));
    EMenu->Add(new MenuBool(
        "AssistManualHook", "Assist player Q with safe E timing", true));
    EMenu->Add(new MenuBool(
        "Peel", "Power Fist the diver threatening protected carry", true));
    EMenu->Add(new MenuBool(
        "Interrupt", "Use E against channels in attack range", true));
    EMenu->Add(new MenuBool(
        "AntiGapcloser", "Use E on committed melee gapclosers", true));
    EMenu->Add(new MenuSeparator(
        "ExactTarget",
        "A wrong minion attack is blocked only while the hooked target will arrive before E expires."));

    RMenu = TacticsMenu->AddSubMenu(new Menu(
        "StaticField", "Shield, silence and passive-mark economy"));
    RMenu->Add(new MenuSlider(
        "MinimumTargets", "Enemies required for ordinary active R", 2, 1, 5));
    RMenu->Add(new MenuBool(
        "ShieldBreak", "Destroy a strategically large damage shield", true));
    RMenu->Add(new MenuSlider(
        "ShieldBreakValue", "Minimum total shield value", 260, 50, 1500));
    RMenu->Add(new MenuBool(
        "Interrupt", "Silence interruptible channels inside R", true));
    RMenu->Add(new MenuBool(
        "MidPullSilence", "Silence escape-ready caster before Q arrival", true));
    RMenu->Add(new MenuBool(
        "PreHookSilence", "Use R-Q on a mobile priority target with clean Q", true));
    RMenu->Add(new MenuBool(
        "Peel", "Use AoE silence for urgent carry peel", true));
    RMenu->Add(new MenuSlider(
        "ExpectedPassiveAttacks", "Likely attacks lost while R is cooling down",
        3, 0, 8));
    RMenu->Add(new MenuBool(
        "Flee", "Use R against multiple close pursuers", true));
    RMenu->Add(new MenuSlider(
        "FleeMinimum", "Pursuers required for flee R", 2, 1, 5));
    RMenu->Add(new MenuSeparator(
        "PassiveEconomy",
        "Pending one-per-second lightning is tracked per target; active R is held when a pending tick already kills."));

    CoachMenu = TacticsMenu->AddSubMenu(new Menu(
        "Coach", "Blitzcrank one-trick geometry and state"));
    CoachMenu->Add(new MenuBool(
        "DrawRanges", "Draw Q, W-E reliability and R ranges", true));
    CoachMenu->Add(new MenuBool(
        "DrawHook", "Draw chosen Q corridor and first body", true));
    CoachMenu->Add(new MenuBool(
        "DrawProtectedAlly", "Mark the dynamically protected ally", true));
    CoachMenu->Add(new MenuBool(
        "DrawETarget", "Mark the exact reserved Power Fist target", true));
    CoachMenu->Add(new MenuBool(
        "DrawMarks", "Show pending R lightning and next detonation", true));
    CoachMenu->Add(new MenuBool(
        "DrawState", "Show posture, sequence, E, passive and ownership", true));
}
...
inline constexpr const char* Scenarios[] = {
    ...
    "Use W to reach an urgent peel target",
