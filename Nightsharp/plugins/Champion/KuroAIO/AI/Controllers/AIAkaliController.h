...
inline bool CastQ(const AIHeroClient& target,
    ...
    const SDK::HitChance minimum = guaranteed || locked
...
inline bool CastE1AtTarget(const AIHeroClient& target,
    ...
    if (prediction.Hitchance < (locked ? SDK::HitChance::Medium : SDK::HitChance::VeryHigh) &&
        !locked) return false;
...
inline void BuildMenu(Menu* root) {
    ...
    PassiveMenu->Add(new MenuBool("HoldForRingExit", "Hold spells while player exits a confirmed passive ring", true));
    PassiveMenu->Add(new MenuSeparator("PlayerMovement",
        "The AI coaches the ring-exit vector; player movement remains player-owned."));

    EnergyMenu = TacticsMenu->AddSubMenu(new Menu(
        "EnergyBudget", "Energy-aware Q and Shroud decisions"));
    EnergyMenu->Add(new MenuBool("UseWForEnergyRoute", "Use W after commitment when it unlocks planned Q/E damage", true));
    ...
    EnergyMenu->Add(new MenuSeparator("NoZeroEnergyAllIn",
        "Q is withheld when the route cannot pay for a follow-up or escape."));

    ShroudMenu = TacticsMenu->AddSubMenu(new Menu(
        "TwilightShroud", "One W: defensive timing and energy battery"));
    ShroudMenu->Add(new MenuBool("UseDefensiveW", "Use W for incoming line/CC, dive, or explicit flee", true));
    ShroudMenu->Add(new MenuSlider("DefensiveWHP", "Defensive W at own HP (%)", 39, 15, 80));
    ShroudMenu->Add(new MenuBool("SaveVsMordekaiser", "Do not spend offensive W into ready Mordekaiser R", true));
    ShroudMenu->Add(new MenuBool("DelayIntoReveal", "Do not spend offensive W into an immediate reveal tool", true));

    FlipMenu = TacticsMenu->AddSubMenu(new Menu(
        "ShurikenFlip", "E1/E2 route selection"));
    FlipMenu->Add(new MenuBool("HarassOnlyGuaranteedE", "Harass E only after Q tip, CC, dash, or committed cast", true));
    FlipMenu->Add(new MenuBool("UseBackflipEntry", "Use safe E-backflip entry just outside Q range", true));
    FlipMenu->Add(new MenuBool("UseShroudAnchor", "Mark shroud with E only for a real route", true));
    FlipMenu->Add(new MenuSlider("TakeMarkBeforeExpiryMs", "Allow safe E2 near mark expiry (ms)", 480, 150, 1100));
    FlipMenu->Add(new MenuSeparator("NoBlindE2",
        "E2 requires visibility, a safe landing, and an exit/kill reason."));

    ExecutionMenu = TacticsMenu->AddSubMenu(new Menu(
        "PerfectExecution", "R1 commitment and R2 economy"));
    ExecutionMenu->Add(new MenuBool("UseR1Engage", "Use R1 for researched assassination entry", true));
    ExecutionMenu->Add(new MenuBool("R1BeforeE2", "R1 before a safe E2 when it creates a superior route", true));
    ExecutionMenu->Add(new MenuBool("UseR1Flee", "Use R1 through a threat only during explicit flee", true));
    ExecutionMenu->Add(new MenuBool("UseR2Execute", "Hold R2 for missing-health execution", true));
    ExecutionMenu->Add(new MenuSlider("R2HealthThreshold", "Nonlethal R2 only below target HP (%)", 44, 10, 85));
    ExecutionMenu->Add(new MenuSlider("NonlethalR2BeforeExpiryMs", "Spend nonlethal R2 only this close to expiry (ms)", 700, 200, 1800));
    ExecutionMenu->Add(new MenuSlider("ExitBeforeExpiryMs", "Reserve R2 exit this long before expiry (ms)", 1200, 450, 2500));
    ExecutionMenu->Add(new MenuSlider("AssassinateTargetHP", "Assassinate posture at target HP (%)", 82, 25, 100));
    ExecutionMenu->Add(new MenuSlider("CleanupTargetHP", "Cleanup posture at target HP (%)", 54, 10, 90));
    ExecutionMenu->Add(new MenuSlider("MinCommitHP", "Minimum own HP for nonlethal commit (%)", 30, 10, 80));
    ExecutionMenu->Add(new MenuSlider("EscapeHP", "Switch to escape posture at own HP (%)", 27, 10, 70));
    ExecutionMenu->Add(new MenuSlider("MaxCommitEnemies", "Maximum enemies at mobility destination", 2, 1, 5));
    ExecutionMenu->Add(new MenuBool("RespectLockdown", "Reject R/E landings into ready point-click lockdown", true));
    ...
    FarmMenu->Add(new MenuBool("ObjectiveE", "Use E1/E2 as a predicted lethal epic-objective secure", true));
    FarmMenu->Add(new MenuSlider("ObjectiveMarkHP", "Mark epic objective below HP (%)", 38, 10, 75));
    FarmMenu->Add(new MenuSlider("ObjectiveDamageBuffer", "Required E2 damage over predicted HP", 35, 0, 200));
    FarmMenu->Add(new MenuSlider("ObjectiveMaxEnemies", "Maximum enemies at objective without W/R2 exit", 2, 0, 5));
    FarmMenu->Add(new MenuSeparator("NoFarmWOrR",
        "W and Perfect Execution are never spent to clear a wave."));

    CoachMenu = TacticsMenu->AddSubMenu(new Menu(
        "AkaliCoach", "Live passive, shroud, E, and R route coaching"));
    CoachMenu->Add(new MenuBool("DrawPassiveRing", "Draw confirmed passive ring and exit vector", true));
    CoachMenu->Add(new MenuBool("DrawShroud", "Draw tracked Shroud center", true));
    CoachMenu->Add(new MenuBool("DrawEBackflip", "Draw planned E1 backflip destination", true));
    CoachMenu->Add(new MenuBool("DrawRRoute", "Draw R1/R2 destinations", true));
    CoachMenu->Add(new MenuBool("DrawState", "Draw energy, passive, recast, and state", true));
}
