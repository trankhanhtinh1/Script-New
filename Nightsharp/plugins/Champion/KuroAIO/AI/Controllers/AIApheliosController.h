...
inline bool EnemyCanDashToSentry(const Vector3& position) {
    for (const auto& enemy : GameObjects::EnemyHeroes()) {
        ...
        if ((_stricmp(name.c_str(), "Yasuo") == 0 &&
             ControllerHelpers::EnemySpellReady(enemy, SDK::SpellSlot::E)) ||
            (_stricmp(name.c_str(), "Samira") == 0 &&
             ControllerHelpers::EnemySpellReady(enemy, SDK::SpellSlot::E)) ||
            (_stricmp(name.c_str(), "Nilah") == 0 &&
             ControllerHelpers::EnemySpellReady(enemy, SDK::SpellSlot::E))) {
            return true;
        }
    }
...
inline void BuildMenu(Menu* root) {
    ...
    TacticsMenu->Add(new MenuBool(
        "KillSecure", "Use a conservatively lethal current-weapon Q", true));
    TacticsMenu->Add(new MenuSlider(
        "HarassMana", "Minimum mana for weapon-aware harass (%)", 48, 0, 100));
    TacticsMenu->Add(new MenuSeparator(
        "Ownership",
        "Movement, ordinary attacks, attack-move, Flash and summoners remain player-owned."));

    WeaponsMenu = TacticsMenu->AddSubMenu(new Menu(
        "Weapons", "Main/off-hand choice and pair value"));
    WeaponsMenu->Add(new MenuSlider(
        "SwapAdvantage", "Required off-hand advantage (x0.01)", 150, 20, 500));
    WeaponsMenu->Add(new MenuBool(
        "StandardCycle", "Maintain Calibrum-Gravitum-Infernum-Severum-Crescendum", true));
    WeaponsMenu->Add(new MenuBool(
        "GreenBlueCycle", "Allow early Calibrum-Infernum alternate cycle", true));
    WeaponsMenu->Add(new MenuBool(
        "ContextHold", "Hold red/purple/blue/white for the fight that is coming", true));
    WeaponsMenu->Add(new MenuSeparator(
        "NoBlindRotation",
        "Cycle maintenance never overrides survival, catch, peel or an active objective."));

    AmmoMenu = TacticsMenu->AddSubMenu(new Menu(
        "Ammo", "Per-gun moonlight and incoming-weapon sequences"));
    AmmoMenu->Add(new MenuBool(
        "LiveReconcile", "Trust live Q ammo only when MaxAmmo identifies a 50-shot gun", true));
    AmmoMenu->Add(new MenuBool(
        "LowAmmoCombos", "Use <=10 ammo Q for a valuable three/four-gun chain", true));
    AmmoMenu->Add(new MenuBool(
        "CancelIncomingWithR", "Allow R to cancel Incoming Weapon for real combat value", true));
    AmmoMenu->Add(new MenuBool(
        "NoLateShowboating", "Reject long late-game swap combos that lose auto DPS", true));
    AmmoMenu->Add(new MenuSeparator(
        "HybridState",
        "Runtime Q/off-hand buffs are authoritative; attack/Q events bridge telemetry gaps."));
    ...
    CalibrumMenu->Add(new MenuBool(
        "QBeforeR", "Land Q before R for sequential marks", true));
    CalibrumMenu->Add(new MenuBool(
        "ConsumeBeforeRefresh", "Let the player consume a live mark before refreshing it", true));
    CalibrumMenu->Add(new MenuBool(
        "BreakShield", "Spend Moonshot into a spell shield", false));
    CalibrumMenu->Add(new MenuBool(
        "PreMarkAutoCoach", "Coach AA-before-mark-reset timing without issuing the AA", true));
    CalibrumMenu->Add(new MenuSeparator(
        "Walls", "Moonshot and mark plans respect Yasuo/Samira/Mel projectile walls."));
    ...
    SeverumMenu->Add(new MenuSlider(
        "SurvivalHp", "Treat Onslaught as survival below HP (%)", 48, 10, 85));
    SeverumMenu->Add(new MenuBool(
        "AntiGapcloser", "Use/swap to Severum against a committed diver", true));
    SeverumMenu->Add(new MenuBool(
        "SwapThroughWalls", "Use non-projectile Severum attacks through projectile walls", true));
    SeverumMenu->Add(new MenuBool(
        "RespectLateAutoDps", "Do not channel when late-game autos are clearly better", true));
    SeverumMenu->Add(new MenuSeparator(
        "OffHand", "Red-white builds chakrams; red-purple tags a root; red R is emergency sustain."));

    GravitumMenu = TacticsMenu->AddSubMenu(new Menu(
        "Gravitum", "Binding Eclipse root discipline"));
    GravitumMenu->Add(new MenuSlider(
        "ProactiveTargets", "Minimum marked champions for proactive root", 2, 1, 5));
    GravitumMenu->Add(new MenuBool(
        "HoldSingle", "Hold a harmless single mark for catch pressure", true));
    GravitumMenu->Add(new MenuBool(
        "RootShield", "Root a lone spell-shielded target", false));
    GravitumMenu->Add(new MenuBool(
        "Interrupt", "Root a marked channel before it completes", true));
    GravitumMenu->Add(new MenuBool(
        "AntiGapcloser", "Root a marked committed dash", true));
    GravitumMenu->Add(new MenuSeparator(
        "Truth", "Binding Eclipse is never cast unless at least one live Gravitum debuff exists."));
    ...
    InfernumMenu->Add(new MenuSlider(
        "ComboTargets", "Minimum Duskwave targets in a grouped combo", 2, 1, 5));
    InfernumMenu->Add(new MenuBool(
        "PreserveObjective", "Hold useful blue ammo for an imminent objective", true));
    InfernumMenu->Add(new MenuBool(
        "AutoResetCoach", "Coach AA-R-Q/AA timing without issuing attacks", true));
    InfernumMenu->Add(new MenuSeparator(
        "Geometry", "Every cone candidate must include the selected target or a better multi-hit set."));
    ...
    CrescendumMenu->Add(new MenuBool(
        "SentryBeforeR", "Place Sentry before Crescendum R so it arms during cast", true));
    CrescendumMenu->Add(new MenuBool(
        "PeelSentry", "Place a safe retreat-zone sentry against a diver", true));
    CrescendumMenu->Add(new MenuBool(
        "HarassSentry", "Spend Sentry for ordinary lane harass", false));
    CrescendumMenu->Add(new MenuBool(
        "ProtectBadAuto", "Replace only zero-stack max-range white autos with a better hand", true));
    CrescendumMenu->Add(new MenuSlider(
        "AvoidZeroStackRange", "Zero-stack white range to prefer off-hand", 590, 450, 700));
    CrescendumMenu->Add(new MenuSeparator(
        "DashTargets", "Never flee-place a turret that gives Yasuo, Samira or Nilah a free dash."));
    ...
    UltimateMenu->Add(new MenuBool(
        "Combo", "Use the best current/off-hand R only above tactical score", true));
    UltimateMenu->Add(new MenuSlider(
        "MinimumScore", "Minimum proactive R score (x0.01)", 680, 200, 1600));
    UltimateMenu->Add(new MenuSlider(
        "InfernumTargets", "Minimum targets for ordinary Infernum R", 2, 1, 5));
    UltimateMenu->Add(new MenuBool(
        "Defensive", "Use Severum/Gravitum R against lethal pressure", true));
    UltimateMenu->Add(new MenuSlider(
        "DefensiveHp", "Defensive R HP threshold (%)", 32, 5, 70));
    UltimateMenu->Add(new MenuSlider(
        "DefensiveScore", "Minimum defensive R score (x0.01)", 520, 100, 1200));
    UltimateMenu->Add(new MenuBool(
        "KillSecure", "Spend R only for a multi-hit or near-certain execute", false));
    UltimateMenu->Add(new MenuSeparator(
        "NoFlash", "The controller never casts Flash; W-R hides the weapon without stealing movement."));
    ...
    FarmMenu->Add(new MenuBool(
        "StopNearEnemy", "Stop automatic ammo preparation near an enemy champion", true));
    FarmMenu->Add(new MenuSlider(
        "InfernumLane", "Minimum lane units for Duskwave", 4, 2, 9));
    FarmMenu->Add(new MenuSlider(
        "InfernumJungle", "Minimum jungle units for Duskwave", 2, 1, 6));
    FarmMenu->Add(new MenuBool(
        "CalibrumLastHit", "Spend 10 Calibrum ammo for a predicted last hit", false));
    FarmMenu->Add(new MenuBool(
        "GravitumDamage", "Use Binding Eclipse damage on a large marked set", false));
    FarmMenu->Add(new MenuSlider(
        "GravitumTargets", "Marked farm units required for Gravitum Q", 4, 2, 10));
    FarmMenu->Add(new MenuBool(
        "CrescendumObjective", "Place Sentry on an epic objective", true));
    ...
    CoachMenu->Add(new MenuBool(
        "DrawRanges", "Draw current Q and attack ranges", true));
    CoachMenu->Add(new MenuBool(
        "DrawMarks", "Draw Calibrum and Gravitum marks", true));
    CoachMenu->Add(new MenuBool(
        "DrawSentries", "Draw sentry range and snapshotted off-hand", true));
    CoachMenu->Add(new MenuBool(
        "DrawUltimate", "Draw last R path and real first-hit explosion", true));
    CoachMenu->Add(new MenuBool(
        "DrawReturn", "Draw the active Crescendum return commitment", true));
    CoachMenu->Add(new MenuBool(
        "DrawState", "Draw pair, queue, ammo, cooldowns, chakrams and plan", true));
}
...
inline constexpr ChampionController Controller = [] {
    ...
    controller.ChampionName = "Aphelios";
