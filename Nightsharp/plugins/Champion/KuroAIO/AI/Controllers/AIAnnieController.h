...
inline void BuildMenu(Menu* root) {
    ...
    TacticsMenu->Add(new MenuBool(
        "PeelBeforeDamage", "Protect a pressured ally before burst", true));
    TacticsMenu->Add(new MenuSeparator(
        "Ownership",
        "Flash, movement, attack-move and Annie positioning remain player-owned."));
    ...
    PassiveMenu->Add(new MenuBool(
        "HiddenQPrime", "At three stacks, Q then E before impact to hide the stun", true));
    PassiveMenu->Add(new MenuBool(
        "HoldStunHarass", "Hold visible stun when a poke target has not committed", true));
    PassiveMenu->Add(new MenuBool(
        "BuildStacks", "Build only to three stacks while safely out of combat", true));
    PassiveMenu->Add(new MenuSlider(
        "BuildMana", "Minimum mana to build passive (%)", 64, 20, 100));
    PassiveMenu->Add(new MenuSeparator(
        "LandingOrder",
        "Q gains on impact; W/E/R gain on cast. The first primed Q/W/R landing consumes stun."));
    ...
    DisintegrateMenu->Add(new MenuBool(
        "Harass", "Use Q for committed short trades", true));
    DisintegrateMenu->Add(new MenuSlider(
        "HarassMana", "Minimum mana for Q harass (%)", 42, 0, 100));
    DisintegrateMenu->Add(new MenuBool(
        "KillSecure", "Use conservatively lethal Q/W automatically", true));
    DisintegrateMenu->Add(new MenuBool(
        "BreakSpellShield", "Use non-primed Q to break a spell shield", true));
    DisintegrateMenu->Add(new MenuBool(
        "SpendStunOnShield", "Allow a primed Q stun to be consumed by a shield", false));
    DisintegrateMenu->Add(new MenuBool(
        "Interrupt", "Use primed Q when it lands before a channel ends", true));
    DisintegrateMenu->Add(new MenuBool(
        "AntiGapcloser", "Use point-click Q stun on a committed dash", true));
    DisintegrateMenu->Add(new MenuBool(
        "Peel", "Use primed Q to peel the protected ally", true));
    DisintegrateMenu->Add(new MenuBool(
        "Flee", "Use primed Q on the closest pursuer", true));
    DisintegrateMenu->Add(new MenuSeparator(
        "Walls", "Q is rejected through Yasuo, Samira and Mel projectile walls."));
    ...
    IncinerateMenu->Add(new MenuBool(
        "Combo", "Use W as the best scored stun cone", true));
    IncinerateMenu->Add(new MenuSlider(
        "StunMinimumTargets", "Minimum targets for proactive W stun", 2, 1, 5));
    IncinerateMenu->Add(new MenuBool(
        "Harass", "Use W harass only when its cone hits multiple targets", false));
    IncinerateMenu->Add(new MenuSlider(
        "HarassMana", "Minimum mana for optional W harass (%)", 68, 35, 100));
    IncinerateMenu->Add(new MenuBool(
        "Interrupt", "Use W stun on a channel in the cone", true));
    IncinerateMenu->Add(new MenuBool(
        "AntiGapcloser", "Use W's faster AoE resolve on a close dash", true));
    IncinerateMenu->Add(new MenuBool(
        "Peel", "Use W cone stun to split a diver from the protected ally", true));
    IncinerateMenu->Add(new MenuBool(
        "Flee", "Use W cone stun while fleeing", true));
    IncinerateMenu->Add(new MenuSeparator(
        "WFlash", "The player presses Flash; W origin is recomputed at cast-end and the controller only continues afterward."));
    ...
    ShieldMenu->Add(new MenuBool(
        "Reactive", "Shield targeted or crossing damage before impact", true));
    ShieldMenu->Add(new MenuSlider(
        "EmergencyHp", "Always override stack hold below ally HP (%)", 38, 5, 80));
    ShieldMenu->Add(new MenuBool(
        "PreserveThree", "Keep three-stack concealment for noncritical shielding", true));
    ShieldMenu->Add(new MenuBool(
        "TradeShield", "Use E around player-led auto short trades", true));
    ShieldMenu->Add(new MenuBool(
        "Peel", "Shield and speed the protected ally against a diver", true));
    ShieldMenu->Add(new MenuSlider(
        "PeelHp", "Protected ally HP for proactive peel E (%)", 58, 10, 95));
    ShieldMenu->Add(new MenuBool(
        "Flee", "Use E for self shield and decaying flee speed", true));
    ...
    TibbersMenu->Add(new MenuBool(
        "Combo", "Use scored R engage when summon value justifies cooldown", true));
    TibbersMenu->Add(new MenuSlider(
        "StunMinimumTargets", "Minimum targets for proactive R stun", 2, 1, 5));
    TibbersMenu->Add(new MenuSlider(
        "MinimumScore", "Minimum scored R-stun placement", 430, 100, 1200));
    TibbersMenu->Add(new MenuSlider(
        "NonStunMinimumScore", "Minimum scored R without stun", 520, 150, 1400));
    TibbersMenu->Add(new MenuBool(
        "KillSecure", "Allow R secure only for multi-hit or near-certain execute", false));
    TibbersMenu->Add(new MenuBool(
        "Interrupt", "Use R stun only for a valuable otherwise-missed interrupt", true));
    TibbersMenu->Add(new MenuBool(
        "AntiGapcloser", "Use R stun for a critical point-blank dash", true));
    TibbersMenu->Add(new MenuBool(
        "Peel", "Summon Tibbers for critical ally peel", true));
    TibbersMenu->Add(new MenuSlider(
        "PeelAllyHp", "Protected ally HP for peel R (%)", 45, 10, 85));
    TibbersMenu->Add(new MenuBool(
        "Flee", "Use R stun as last-resort disengage", true));

    PetMenu = TacticsMenu->AddSubMenu(new Menu(
        "TibbersMicro", "Soft pet autopilot with manual ownership"));
    PetMenu->Add(new MenuBool(
        "SoftAutopilot", "Focus clear targets but yield after player R orders", true));
    PetMenu->Add(new MenuBool(
        "AllowPetDive", "Allow Tibbers-only turret pursuit at low target HP", false));
    PetMenu->Add(new MenuSlider(
        "PetDiveTargetHp", "Maximum target HP for optional pet dive (%)", 22, 5, 50));
    PetMenu->Add(new MenuSlider(
        "ReturnLeash", "Return Tibbers beyond this owner distance", 1050, 700, 1400));
    PetMenu->Add(new MenuSlider(
        "CriticalLeash", "Force return beyond this owner distance", 1450, 1000, 1900));
    PetMenu->Add(new MenuBool(
        "FarmWithPet", "Use Tibbers on the current wave or jungle camp", true));
    PetMenu->Add(new MenuSeparator(
        "ManualLimit",
        "SDK has no inbound Alt-click event; orders are throttled and every observed manual R command receives a long lock."));

    FarmMenu = TacticsMenu->AddSubMenu(new Menu(
        "Farm", "Q-refund lane and conservative cone farming"));
    FarmMenu->Add(new MenuBool(
        "UseQ", "Last-hit with Q only when predicted to refund", true));
    FarmMenu->Add(new MenuBool(
        "UseW", "Use W only on a large wave or valuable jungle camp", true));
    FarmMenu->Add(new MenuBool(
        "PreserveStun", "Do not spend visible stun while an enemy champion is near", true));
    ...
    CoachMenu->Add(new MenuBool(
        "DrawQ", "Draw Q range, missile target and stun race", true));
    CoachMenu->Add(new MenuBool(
        "DrawW", "Draw W cone from its actual resolve origin", true));
    CoachMenu->Add(new MenuBool(
        "DrawR", "Draw R cast range and best summon circle", true));
    CoachMenu->Add(new MenuBool(
        "DrawPet", "Draw Tibbers enrage and current focus", true));
    CoachMenu->Add(new MenuBool(
        "DrawPeel", "Draw protected ally and selected diver", true));
    CoachMenu->Add(new MenuBool(
        "DrawState", "Draw posture, stack, landing race, E and pet state", true));
}
