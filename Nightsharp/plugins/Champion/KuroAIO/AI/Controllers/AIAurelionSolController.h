...
inline RPlan BuildRPlan(const AIBaseClient& primary,
    ...
    for (const Vector3& raw : candidates) {
        ...
        if (!manual && !CursorDirectionAgrees(requested, -0.24f) &&
            ...
            purpose != RPurpose::AntiGapcloser) score -= 360.0f;
        if (score > best.Score) {
...
inline bool TryFarm(Mode mode) {
    const bool jungle = mode == Mode::Jungle ||
        (mode == Mode::LaneClear && HasNearbyJungleTarget(
             ERange(ObjectManager::Player().Level(), false)) &&
    ...
    if (IsREvent(args)) {
        ...
        if (ProjectileWallFirstContactFromPlayer(
            ...
            RImpactTick = now + static_cast<int>(std::min(
                ...
                0.20f + ObjectManager::Player().Position().Distance2D(
...
inline void BuildMenu(Menu* root) {
    ...
    TacticsMenu->Add(new MenuBool(
        "ProtectWinCondition", "Front-to-back: peel the allied win condition", true));
    TacticsMenu->Add(new MenuSlider(
        "AllyPeelHealth", "Peel ally below health (%)", 52, 10, 90));
    TacticsMenu->Add(new MenuBool(
        "KillSecure", "Use exact Q/R packages to secure a kill", true));
    TacticsMenu->Add(new MenuSeparator(
        "Ownership",
        "Movement, cursor steering, attacks and summoners remain player-owned."));
    ...
    BreathMenu->Add(new MenuBool(
        "Combo", "Channel only with a verified one-second primary window", true));
    BreathMenu->Add(new MenuBool(
        "Harass", "Pressure lane after blocker and cursor agreement", true));
    BreathMenu->Add(new MenuBool(
        "SplashTap", "Use a short Q splash tap through the wave", true));
    BreathMenu->Add(new MenuSlider(
        "SplashTapMana", "Minimum mana for splash tap (%)", 72, 0, 100));
    BreathMenu->Add(new MenuSlider(
        "HarassMana", "Minimum mana for Q harass (%)", 52, 0, 100));
    BreathMenu->Add(new MenuSlider(
        "FlatReserve", "Mana kept after the planned channel", 45, 0, 250));
    BreathMenu->Add(new MenuBool(
        "ProtectChannel", "Block attacks that would break active Q", true));
    BreathMenu->Add(new MenuSeparator(
        "Steering",
        "The controller chooses whether to begin/release Q; the player's cursor steers it."));
    ...
    FlightMenu->Add(new MenuBool(
        "Combo", "Use an offset flight during committed all-ins", true));
    FlightMenu->Add(new MenuBool(
        "OnlyAfterCCSpent", "Engage only after key CC is spent or target is locked", true));
    FlightMenu->Add(new MenuBool(
        "ContinueActiveBreath", "Convert active Q into Q-W continuous breath", true));
    FlightMenu->Add(new MenuBool(
        "PlanWithBreathSlow", "Score routes at half speed for W-Q", true));
    FlightMenu->Add(new MenuBool(
        "Flee", "Fly on a cursor-aligned terrain-separated escape", true));
    FlightMenu->Add(new MenuSlider(
        "MaximumRoute", "Maximum automatic combat flight length", 1350, 650, 2500));
    FlightMenu->Add(new MenuSlider(
    ...
    SingularityMenu->Add(new MenuBool(
        "Combo", "Place E on the target exit before Q/R", true));
    SingularityMenu->Add(new MenuBool(
        "FlightDrop", "Drop E from the safe side of W", true));
    SingularityMenu->Add(new MenuBool(
        "Harass", "Use E only when champion and stack value overlap", true));
    ...
    SingularityMenu->Add(new MenuBool(
        "AntiGapcloser", "Place E on a directed gapcloser endpoint", true));
    SingularityMenu->Add(new MenuBool(
        "InterruptZone", "Use E to disrupt a long channel when R is late", true));
    SingularityMenu->Add(new MenuBool(
        "AllyPeel", "Pull a diver away from the protected carry", true));
    SingularityMenu->Add(new MenuBool(
        "Flee", "Drop E between Aurelion Sol and the pursuer", true));

    StarMenu = TacticsMenu->AddSubMenu(new Menu(
        "FallingStar", "R normal/Skies Descend impact planner"));
    StarMenu->Add(new MenuBool(
        "Combo", "Layer R inside E or a multi-target window", true));
    StarMenu->Add(new MenuSlider(
        "RegularMinimumHits", "Minimum regular R direct hits", 2, 1, 5));
    StarMenu->Add(new MenuSlider(
        "EmpoweredMinimumHits", "Minimum empowered R direct hits", 2, 1, 5));
    StarMenu->Add(new MenuBool(
        "ReserveForObjective", "Preserve Skies Descend for objective fights", true));
    StarMenu->Add(new MenuBool(
        "Interrupt", "R only when its real impact beats the channel", true));
    StarMenu->Add(new MenuBool(
        "AntiGapcloser", "Spend R for critical self peel", true));
    StarMenu->Add(new MenuBool(
        "AllyPeel", "Use R to protect the allied win condition", true));
    StarMenu->Add(new MenuBool(
        "KillSecure", "Use exact mitigated direct damage", true));
    StarMenu->Add(new MenuBool(
        "Flee", "Use R before W only for committed peel", true));
    StarMenu->Add(new MenuSlider(
    ...
    StarMenu->Add(new MenuSeparator(
        "WallImpact",
        "Wind Wall, Blade Whirl and Rebuttal relocate R impact; empowered shockwave still resolves."));
    ...
    FarmMenu->Add(new MenuBool(
        "UseELane", "Use E only on a valuable dying wave", true));
    FarmMenu->Add(new MenuBool(
        "UseQLane", "Focus cannon/large body so splash clears the wave", true));
    FarmMenu->Add(new MenuSlider(
        "ELaneStacks", "Expected lane E Stardust", 6, 1, 10));
    FarmMenu->Add(new MenuSlider(
        "ELaneUnits", "Minimum lane units in E", 5, 1, 10));
    FarmMenu->Add(new MenuSlider(
        "LaneMana", "Minimum mana for lane harvest (%)", 62, 0, 100));
    FarmMenu->Add(new MenuBool(
        "UseEJungle", "Use E on durable camps/objectives", true));
    FarmMenu->Add(new MenuBool(
        "UseQJungle", "Hold Q on the selected jungle body", true));
    FarmMenu->Add(new MenuSlider(
        "EJungleStacks", "Expected jungle E Stardust", 3, 1, 10));
    FarmMenu->Add(new MenuSlider(
        "EJungleUnits", "Minimum jungle units in E", 2, 1, 8));
    FarmMenu->Add(new MenuSlider(
        "JungleMana", "Minimum mana for jungle harvest (%)", 38, 0, 100));

    CoachMenu = TacticsMenu->AddSubMenu(new Menu(
        "Coach", "One-trick geometry and state visualization"));
    CoachMenu->Add(new MenuBool(
        "DrawRanges", "Draw live level-scaled Q/E and R ranges", true));
    CoachMenu->Add(new MenuBool(
        "DrawBeam", "Draw chosen beam and actual first body", true));
    CoachMenu->Add(new MenuBool(
        "DrawFlight", "Draw selected W route and stop endpoint", true));
    CoachMenu->Add(new MenuBool(
        "DrawSingularity", "Draw E damage and execute centers", true));
    CoachMenu->Add(new MenuBool(
        "DrawStar", "Draw actual R impact after projectile barriers", true));
    CoachMenu->Add(new MenuBool(
        "DrawPeel", "Draw protected ally and current diver", true));
    CoachMenu->Add(new MenuBool(
        "DrawState", "Draw Stardust, R cycle and burst continuity", true));
}
...
inline constexpr const char* Scenarios[] = {
    ...
    "Place E on a directed gapcloser endpoint",
