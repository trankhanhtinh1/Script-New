...
inline void BuildMenu(Menu* root) {
    ...
    TacticsMenu->Add(new MenuBool(
        "KillSecure", "Use exact Q/E/R geometry to secure kills", true));
    TacticsMenu->Add(new MenuSlider(
        "DefensiveHealth", "Peel R health threshold (%)", 42, 10, 90));
    TacticsMenu->Add(new MenuSlider(
        "ManualOwnershipMs", "Yield after a player-cast spell (ms)",
        520, 180, 1100));
    TacticsMenu->Add(new MenuSeparator(
        "Ownership",
        "Movement, attack commands, cursor, Flash and passive clicks remain player-owned."));
    ...
    SoldierMenu->Add(new MenuBool(
        "ReserveForE", "Keep one W charge while E is available", true));
    SoldierMenu->Add(new MenuBool(
        "PreserveEscapeAnchor", "Do not Q away the only E anchor", true));
    SoldierMenu->Add(new MenuSeparator(
        "Orbwalker",
        "OrbwalkerKuro now attacks, kites, last-hits and clears through commandable soldiers."));
    ...
    QMenu->Add(new MenuSeparator(
        "LateQ",
        "Q follows the first soldier auto and waits until coverage is leaving; it never cancels a valuable windup."));

    WMenu = TacticsMenu->AddSubMenu(new Menu(
        "Arise", "Soldier placement and charge economy"));
    WMenu->Add(new MenuBool(
        "DefensiveAnchor", "Place a safe anchor against committed threats", true));
    WMenu->Add(new MenuBool(
        "FollowupAfterR", "Place a soldier on the R landing path", true));
    WMenu->Add(new MenuBool(
        "FleeAnchor", "Place W toward the player's flee cursor", true));

    EMenu = TacticsMenu->AddSubMenu(new Menu(
        "ShiftingSands", "First-collision dash and E-Q drift"));
    EMenu->Add(new MenuBool(
        "CollisionRefund", "Use verified champion collision for W refund", true));
    EMenu->Add(new MenuBool(
        "ReactiveEscape", "E to a safe soldier against gapclosers/CC", true));
    EMenu->Add(new MenuBool(
        "Flee", "Use cursor-agreeing W-E-Q while fleeing", true));
    EMenu->Add(new MenuSeparator(
        "Projectile",
        "E is rejected across live projectile walls and stops on the first champion body."));

    RMenu = TacticsMenu->AddSubMenu(new Menu(
        "EmperorsDivide", "Peel-first R and gated shuffle policy"));
    RMenu->Add(new MenuBool(
        "ReactivePeel", "R committed divers and interruptible channels", true));
    RMenu->Add(new MenuBool(
        "MultiTarget", "Use R for verified multi-target value", true));
    RMenu->Add(new MenuBool(
        "Pick", "Push a low target toward allied follow-up", true));
    RMenu->Add(new MenuBool(
        "KillSecure", "Use mitigated R damage only when lethal", true));
    RMenu->Add(new MenuBool(
        "FleePeel", "Use R as the final flee peel", true));
    RMenu->Add(new MenuSlider(
        "MinimumHits", "Minimum ordinary offensive R hits", 2, 1, 5));
    RMenu->Add(new MenuBool(
        "AutomaticShuffle", "Automatically shuffle only a verified opportunity", false));
    RMenu->Add(new MenuSlider(
        "ShuffleMinimumEnemies", "Enemies required to justify a shuffle",
        3, 2, 5));
    ...
    RMenu->Add(new MenuSeparator(
        "NoFlash",
        "Flash extensions are coached but never cast by the controller."));

    PassiveMenu = TacticsMenu->AddSubMenu(new Menu(
        "ShurimasLegacy", "Sun Disc macro advisory"));
    PassiveMenu->Add(new MenuBool(
        "SuggestSunDisc", "Draw a safe objective/side-lane Sun Disc window", true));
    PassiveMenu->Add(new MenuSeparator(
        "AdvisoryOnly",
        "The passive requires a player-selected ruin and channel; this controller never clicks it."));

    FarmMenu = TacticsMenu->AddSubMenu(new Menu(
        "Farm", "Soldier formation without wasting mobility"));
    FarmMenu->Add(new MenuBool(
        "HoldForChampion", "Hold farm spells while a champion contests", true));
    FarmMenu->Add(new MenuSlider(
        "ChampionHoldRange", "Champion contest range", 1150, 500, 1800));
    FarmMenu->Add(new MenuBool(
        "UseW", "Place a soldier on valuable lane/jungle clusters", true));
    FarmMenu->Add(new MenuBool(
        "UseQ", "Re-form existing soldiers across valuable clusters", true));
    FarmMenu->Add(new MenuSlider(
    ...
    FarmMenu->Add(new MenuSeparator(
        "LastHit",
        "Last-hit is delegated to OrbwalkerKuro's exact Sand Soldier damage model; E and R are never farming tools."));

    CoachMenu = TacticsMenu->AddSubMenu(new Menu(
        "Coach", "One-trick state and geometry visualization"));
    CoachMenu->Add(new MenuBool(
        "DrawRanges", "Draw W, soldier command and Q ranges", true));
    CoachMenu->Add(new MenuBool(
        "DrawSoldiers", "Draw commandable soldier attack zones", true));
    CoachMenu->Add(new MenuBool(
        "DrawQFormation", "Draw every selected Q endpoint", true));
    CoachMenu->Add(new MenuBool(
        "DrawDrift", "Draw E anchor and E-Q endpoint", true));
    CoachMenu->Add(new MenuBool(
        "DrawR", "Draw R push direction and final wall", true));
    CoachMenu->Add(new MenuBool(
        "DrawState", "Draw posture, sequence, ammo and ownership", true));
}
