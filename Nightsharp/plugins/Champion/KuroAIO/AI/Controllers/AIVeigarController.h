...
inline bool CastE(const AIHeroClient& target, Mode mode, bool reactive = false) {
    ...
    if (!CageSafePlacement(center, player.Position(),
                           ...
                           Engine::UnderEnemyTurret(center), reactive) ||
        ControllerHelpers::ProjectileWallBlocksFromPlayer(center, kECageRadius * 0.08f)) return false;
...
inline bool CastW(const AIHeroClient& target, Mode mode, bool reactive = false) {
    ...
    if (predicted.IsZero() || player.Position().Distance2D(predicted) > kWRange + 50.0f ||
        !WImpactHits(predicted, predicted, target.BoundingRadius()) ||
        ControllerHelpers::ProjectileWallBlocksFromPlayer(predicted, 1.0f)) return false;
...
inline bool CastR(const AIHeroClient& target, Mode mode, bool reactive = false) {
    ...
    const ExecuteContext context{
        ...
        ControllerHelpers::ProjectileWallBlocksFromPlayer(target.Position(), 40.0f),
...
inline bool TryFarm(Mode mode) {
    ...
    if (plan.Valid && plan.Hits >= minimum && (!lastHit || plan.LastHits > 0) &&
        !PreserveAttack(0, false) && Engine::ControllerCastPosition(0, plan.Aim)) {
        LastCastTick[0] = Now();
        return true;
    }
...
inline constexpr ChampionController Controller = [] {
    ...
    controller.ChampionName = "Veigar";
