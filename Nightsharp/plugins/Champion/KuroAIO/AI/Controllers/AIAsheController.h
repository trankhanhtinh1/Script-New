...
inline bool IsPerHitFlatReductionTarget(const AIBaseClient& target) {
    ...
    const std::string champion = target.CharacterName();
    if (_stricmp(champion.c_str(), "Amumu") == 0 ||
        _stricmp(champion.c_str(), "Fizz") == 0) {
        return true;
    }
    if (_stricmp(champion.c_str(), "Leona") == 0 &&
...
inline void BuildMenu(Menu* root) {
    ...
    TacticsMenu->Add(new MenuBool(
        "AllyPeel", "Peel a pressured carry before adding damage", true));
    TacticsMenu->Add(new MenuSeparator(
        "Ownership",
        "Movement, attack-move, target selection and summoners stay player-owned."));

    FocusMenu = TacticsMenu->AddSubMenu(new Menu(
        "RangersFocus", "Q four-stack reset and flurry discipline"));
    FocusMenu->Add(new MenuBool(
        "Combo", "AA-Q-AA only in a sustained combo window", true));
    FocusMenu->Add(new MenuBool(
        "Harass", "Reset a player-led extended short trade", true));
    FocusMenu->Add(new MenuBool(
        "Wave", "Use after an auto only while four wave attacks remain", true));
    FocusMenu->Add(new MenuBool(
        "Jungle", "Use after an auto on a durable ordinary camp", true));
    FocusMenu->Add(new MenuBool(
        "Objective", "Use after an auto on epic monsters", true));
    FocusMenu->Add(new MenuBool(
        "Structures", "Use after an auto while safely hitting a structure", true));
    FocusMenu->Add(new MenuBool(
        "RespectFlatReduction", "Hold short flurries into per-hit flat reduction", true));
    FocusMenu->Add(new MenuSeparator(
        "Reset",
        "Q is never pre-cast: the after-attack event owns AA-Q-AA timing."));

    VolleyMenu = TacticsMenu->AddSubMenu(new Menu(
        "Volley", "W live 7-11 ray fan and first-blocker solver"));
    VolleyMenu->Add(new MenuBool(
        "Combo", "Use a clear ray for chase, setup or multi-hit", true));
    VolleyMenu->Add(new MenuBool(
        "Harass", "Thread a side ray through the wave for poke", true));
    VolleyMenu->Add(new MenuSlider(
        "HarassMana", "Minimum mana for W harass (%)", 48, 0, 100));
    VolleyMenu->Add(new MenuBool(
        "SelfPeel", "Slow a committed diver before spending R", true));
    VolleyMenu->Add(new MenuBool(
        "AntiGapcloser", "Use the best clear W ray on a directed dash", true));
    VolleyMenu->Add(new MenuBool(
        "KillSecure", "Use only a predicted lethal unblocked W ray", true));
    VolleyMenu->Add(new MenuSeparator(
        "Geometry",
        "Every rank-dependent five-degree ray resolves its own first hostile blocker; one unit receives damage once."));
    ...
    HawkshotMenu->Add(new MenuBool(
        "Automatic", "Allow conservative Summoner's Rift scouting", true));
    HawkshotMenu->Add(new MenuBool(
        "FirstClear", "Scout multi-camp enemy first-clear routes", true));
    HawkshotMenu->Add(new MenuBool(
        "TrackJungler", "Project from the last seen Smite holder", true));
    HawkshotMenu->Add(new MenuBool(
        "Objectives", "Scout objective paths before committing", true));
    HawkshotMenu->Add(new MenuBool(
        "AtChargeCap", "Spend a strong route before wasting recharge", true));
    ...
    HawkshotMenu->Add(new MenuSlider(
        "RepeatSeconds", "Do not repeat a landmark for (seconds)", 24, 8, 60));
    HawkshotMenu->Add(new MenuSlider(
        "DecisionSeconds", "Minimum time between scout decisions", 12, 5, 40));
    HawkshotMenu->Add(new MenuSeparator(
        "MapScope",
        "Automatic landmarks run only on Summoner's Rift; manual cursor scouting works on every map."));
    ...
    ArrowMenu->Add(new MenuSlider(
        "ManualCursorRadius", "Manual target radius around cursor", 700, 150, 1600));
    ArrowMenu->Add(new MenuBool(
        "Combo", "Use R for a verified pick or clustered teamfight", true));
    ArrowMenu->Add(new MenuBool(
        "AfterVolley", "Use W slow as a committed R confirmation", true));
    ArrowMenu->Add(new MenuSlider(
        "PickAllies", "Minimum allied follow-up for a pick", 1, 0, 3));
    ArrowMenu->Add(new MenuSlider(
        "PickStunTenths", "Minimum pick stun duration (tenths)", 20, 10, 35));
    ArrowMenu->Add(new MenuSlider(
        "TeamfightHits", "Minimum R explosion targets", 3, 2, 5));
    ArrowMenu->Add(new MenuBool(
        "CrossMap", "Allow strict automatic cross-map picks", false));
    ArrowMenu->Add(new MenuBool(
        "AllowLongProactive", "Allow non-manual plans beyond 2500 range", false));
    ArrowMenu->Add(new MenuBool(
        "Interrupt", "Interrupt only when R arrives before channel end", true));
    ArrowMenu->Add(new MenuBool(
        "AntiGapcloser", "Use point-blank R as critical hard peel", true));
    ArrowMenu->Add(new MenuBool(
        "SelfPeel", "Use R after W cannot safely disengage", true));
    ArrowMenu->Add(new MenuSlider(
        "SelfPeelHp", "Player HP for defensive R (%)", 52, 10, 90));
    ArrowMenu->Add(new MenuSlider(
        "AllyPeelHp", "Protected ally HP for defensive R (%)", 48, 10, 85));
    ArrowMenu->Add(new MenuBool(
        "KillSecure", "Allow exact R execute under strict range/follow-up", false));
    ArrowMenu->Add(new MenuSlider(
        "KillSecureRange", "Maximum solo R execute range", 1800, 700, 5000));
    ArrowMenu->Add(new MenuSeparator(
        "Collision",
        "R ignores minions, stops on the first enemy champion, accelerates and scales stun with distance."));

    FarmMenu = TacticsMenu->AddSubMenu(new Menu(
        "Farm", "Player-led flurry and exact Volley farming"));
    FarmMenu->Add(new MenuBool(
        "UseW", "Use the fan only on a valuable wave or camp", true));
    FarmMenu->Add(new MenuBool(
        "HoldWForChampion", "Preserve W while an enemy champion can contest", true));
    FarmMenu->Add(new MenuSlider(
        "WWaveHits", "Minimum unique lane targets for W", 5, 3, 10));
    FarmMenu->Add(new MenuSlider(
    ...
    CoachMenu->Add(new MenuBool(
        "DrawVolley", "Draw every live Volley ray and chosen ray", true));
    CoachMenu->Add(new MenuBool(
        "DrawHawkshot", "Draw latest scout route and destination vision", true));
    CoachMenu->Add(new MenuBool(
        "DrawArrow", "Draw first-hit R path and explosion", true));
    CoachMenu->Add(new MenuBool(
        "DrawPeel", "Draw protected ally and diver", true));
    CoachMenu->Add(new MenuBool(
        "DrawState", "Draw posture, Focus, E charges and R plan", true));
}
