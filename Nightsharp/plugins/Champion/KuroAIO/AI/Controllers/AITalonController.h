...
inline bool HasProjectileWall(const Vector3& start, const Vector3& end) {
    ...
    for (float step = 20.0f; step < distance - 20.0f; step += 18.0f) {
        if (SDK::NavMesh::IsWall(start + direction * step)) return true;
    }
    return false;
}
...
inline bool CastQ(const AIHeroClient& target, Mode mode, bool defensive = false) {
    ...
    if (!melee && HasProjectileWall(player.Position(), target.Position())) return false;
...
inline bool CastW(const AIHeroClient& target, Mode mode, bool defensive = false) {
    ...
    if (endpoint.IsZero() || player.Position().Distance2D(endpoint) > kWOutboundRange + target.BoundingRadius() ||
        prediction.Hitchance < SDK::HitChance::High || HasProjectileWall(player.Position(), endpoint)) return false;
