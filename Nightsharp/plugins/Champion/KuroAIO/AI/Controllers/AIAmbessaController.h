...
inline void BuildMenu(Menu* root) {
    ...
    TacticsMenu->Add(new MenuSlider(
        "AllInHp", "Target HP for all-in posture (%)", 54, 15, 90));
    TacticsMenu->Add(new MenuSlider(
    ...
    TacticsMenu->Add(new MenuSeparator(
        "Ownership",
        "Abilities are assisted; movement, Hold/Stop and Flash remain player-owned."));
    ...
    PassiveMenu->Add(new MenuBool(
        "AlwaysWeaveInRange", "Weave empowered AA when a champion is in range", true));
    PassiveMenu->Add(new MenuBool(
        "SkipAAForBurst", "Skip AA when QE/Q2 must land immediately", true));
    PassiveMenu->Add(new MenuBool(
        "ProtectCombatStacks", "Do not spend combat stacks on a wrong unit", true));
    PassiveMenu->Add(new MenuBool(
        "KeepQSweetspot", "Choose no-dash when moving would lose Q1 edge", true));
    PassiveMenu->Add(new MenuBool(
        "UseQuickQE", "Use the specialist no-dash QE animation cancel", true));
    PassiveMenu->Add(new MenuSlider(
        "QuickQEDelay", "Earliest E after Q for quick cancel (ms)", 185, 150, 260));
    PassiveMenu->Add(new MenuSlider(
        "QuickQEHp", "Target HP for quick QE branch (%)", 38, 10, 75));
    PassiveMenu->Add(new MenuBool(
        "RespectDashHazards", "Avoid ready anti-dash zones at Step endpoint", true));
    PassiveMenu->Add(new MenuBool(
        "BaitAntiDash", "Prefer no-dash while Poppy/Taliyah/Cassiopeia holds denial", true));
    PassiveMenu->Add(new MenuSlider(
        "MaxDashEnemies", "Maximum enemies around a passive-dash endpoint", 2, 1, 5));
    PassiveMenu->Add(new MenuSlider(
        "FleeReserve", "Energy retained after a flee spell", 70, 0, 140));
    PassiveMenu->Add(new MenuSeparator(
        "HoldStop",
        "The coach marks a no-dash branch; it never presses Hold or Stop for the player."));

    QMenu = TacticsMenu->AddSubMenu(new Menu(
        "CunningSweep", "Q1 blade edge and Q2 first-target control"));
    QMenu->Add(new MenuBool(
        "RequireEdge", "Hold Q1 body hit unless it is lethal/all-in", true));
    QMenu->Add(new MenuBool(
        "RequireFirstTarget", "Hold Q2 when another unit would take full damage", true));
    QMenu->Add(new MenuSlider(
        "Q2ExpiryMs", "Force Q2 before recast expires (ms)", 380, 180, 700));
    QMenu->Add(new MenuBool(
        "ProtectExpiringQ2", "Block AA whose impact would lose an available Q2", true));
    QMenu->Add(new MenuBool(
        "UseForFleeStep", "Spend Q for a player-directed flee Step", true));
    QMenu->Add(new MenuSeparator(
        "QGeometry",
        "Q1: 180 degree, 275 body/400 edge. Q2: 650 x 80 and first unit takes full damage."));

    WMenu = TacticsMenu->AddSubMenu(new Menu(
        "Repudiation", "Shield interception and empowered slam"));
    WMenu->Add(new MenuBool(
        "ReactiveShield", "Time W into champion, turret or large-monster damage", true));
    WMenu->Add(new MenuSlider(
        "DamagePercent", "Minimum incoming damage (% max HP)", 11, 4, 40));
    WMenu->Add(new MenuSlider(
        "MaximumLeadMs", "Maximum W lead before predicted impact (ms)", 520, 120, 900));
    WMenu->Add(new MenuBool(
        "ShieldGapcloser", "Shield a committed gapcloser", true));
    WMenu->Add(new MenuBool(
        "UseInAllIn", "Use W in all-in only around counter-damage", true));
    WMenu->Add(new MenuSlider(
        "AllInHp", "Player HP allowing proactive all-in W (%)", 58, 20, 90));
    WMenu->Add(new MenuBool(
    ...
    WMenu->Add(new MenuSeparator(
        "Empower",
        "W is counted as 150% damage only after a valid incoming hit is observed."));
    ...
    EMenu->Add(new MenuBool("UseInHarass", "Use E to slow a close harass target", true));
    EMenu->Add(new MenuBool("UseInFlee", "E a pursuer before player dashes away", true));
    EMenu->Add(new MenuBool("SlowGapcloser", "E a gapcloser at its endpoint", true));
    EMenu->Add(new MenuSeparator(
        "SecondHit",
        "The second E is projected only after AmbessaPassiveDash/path movement is observed."));

    RMenu = TacticsMenu->AddSubMenu(new Menu(
        "PublicExecution", "Farthest-target selection and landing safety"));
    RMenu->Add(new MenuBool("Execute", "R only when R is uniquely required to execute", true));
    RMenu->Add(new MenuBool("IsolateCarry", "R a carry into a supported all-in", true));
    RMenu->Add(new MenuSlider("IsolateHp", "Carry HP for isolation (%)", 72, 25, 100));
    RMenu->Add(new MenuBool("LongCatch", "R a cursor-approved distant catch", true));
    ...
    RMenu->Add(new MenuBool(
        "UnstoppableDodge", "Use R cast/lockout to ignore dangerous incoming CC", true));
    RMenu->Add(new MenuSlider(
        "DodgeLeadMs", "Maximum incoming-CC lead for R (ms)", 760, 180, 1000));
    RMenu->Add(new MenuSlider(
        "DodgeHp", "Player HP for R CC-dodge (%)", 58, 20, 90));
    RMenu->Add(new MenuSlider(
        "MaxLandingEnemies", "Maximum enemies around normal R landing", 2, 1, 5));
    RMenu->Add(new MenuSeparator(
        "FarthestRule",
        "Every aim is simulated against all champions; a farther body can steal the cast."));
    ...
    FarmMenu->Add(new MenuBool("JungleE", "Use E with a player-owned kite dash", true));
    FarmMenu->Add(new MenuBool("JungleW", "Counter an epic-monster hit with W", true));
    ...
    FarmMenu->Add(new MenuSeparator(
        "MonsterData",
        "Monster Q adds current 75 flat and caps max-health damage at 100-300 by level."));

    CoachMenu = TacticsMenu->AddSubMenu(new Menu(
        "Coach", "One-trick visual coaching"));
    CoachMenu->Add(new MenuBool("DrawQ1", "Draw Q1 body and blade-edge radii", true));
    CoachMenu->Add(new MenuBool("DrawQ2", "Draw Q2 first-target plan", true));
    CoachMenu->Add(new MenuBool("DrawDash", "Draw player-owned Step endpoint", true));
    CoachMenu->Add(new MenuBool("DrawR", "Draw R line, selected target and landing", true));
    CoachMenu->Add(new MenuBool("DrawState", "Draw posture, energy, stacks and live branch", true));
}
