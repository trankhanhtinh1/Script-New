...
inline bool HasProjectileWall(const Vector3& origin, const Vector3& end) {
    const float distance = origin.Distance2D(end);
    if (distance <= 1.0f) return false;
    const Vector3 direction = Direction2D(origin, end);
    for (float step = 18.0f; step < distance - 18.0f; step += 20.0f) {
        if (SDK::NavMesh::IsWall(origin + direction * step)) return true;
    }
    return false;
}
...
inline bool CastR(const AIHeroClient& target, Mode mode, bool defensive = false) {
    ...
    const RContext context{ true, true, line.Hit, HasProjectileWall(player.Position(), predicted),
