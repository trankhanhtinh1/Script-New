...
inline bool CastBubble(const AIHeroClient& target, Mode mode, bool reactive = false,
    ...
    const bool wallOnPath = ControllerHelpers::ProjectileWallBlocksFromPlayer(
        aim, kEWidth * 0.5f);
    const bool throughWall = wallOnPath;
    if (wallOnPath && !Bool(BubbleMenu, "AllowWallBubbles", true)) return false;
...
inline void BuildMenu(Menu* root) {
    ...
    BubbleMenu->Add(new MenuBool("AllowWallBubbles", "Allow wall-extended bubbles", true));
