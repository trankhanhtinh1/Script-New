...
inline bool CastStance(Stance stance, const AIHeroClient& target, Mode mode,
    ...
    if (stance == Stance::WildingClaw) {
        if (Protected(target) ||
            ...
            ProjectileWallBlocksFromPlayer(PredictPosition(target, 0.10f), 0.0f)) return false;
        ...
        if (Protected(target) ||
            !StormTargetValid(player.Position(), PredictPosition(target, 0.15f),
                              target.BoundingRadius(), ProjectileWallBlocksFromPlayer(
                                  PredictPosition(target, 0.15f), 0.0f))) return false;
