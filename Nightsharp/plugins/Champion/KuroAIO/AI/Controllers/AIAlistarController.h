...
inline bool TryJungle() {
    if (!Bool(FarmMenu, "JungleAbilities", false) ||
        Engine::CountEnemiesAt(ObjectManager::Player().Position(), 1200.0f) > 0) {
        return false;
    }
    AIBaseClient best{};
...
inline Posture ChoosePosture(Mode mode,
    ...
    if (Engine::CountEnemiesAt(ObjectManager::Player().Position(),
                           kQRadius + 65.0f) >= 2) {
...
inline void BuildMenu(Menu* root) {
    ...
    RoleMenu->Add(new MenuBool("ProtectCarry", "Peel a threatened carry before engaging", true));
    RoleMenu->Add(new MenuSlider("PeelThreatScore", "Minimum carry-threat score", 620, 250, 1100));
    RoleMenu->Add(new MenuSlider("MinimumFollowup", "Minimum allies able to follow W-Q", 1, 0, 4));
    RoleMenu->Add(new MenuSlider("MaxEngageEnemies", "Maximum enemies at engage destination", 3, 1, 5));
    RoleMenu->Add(new MenuBool("RespectCursor", "Do not hard-engage opposite the player cursor", true));
    RoleMenu->Add(new MenuBool("DoNotOverlapCC", "Wait instead of overlapping existing hard CC", true));
    RoleMenu->Add(new MenuBool("UseCCForPassiveHeal", "At 6 passive stacks, CC to heal a low ally", true));
    RoleMenu->Add(new MenuSlider("PassiveHealAllyHp", "Low-ally HP for passive setup (%)", 42, 15, 75));
    ...
    HeadbuttMenu->Add(new MenuBool("UseWQ", "Use buffered W-Q only for a supported engage", true));
    HeadbuttMenu->Add(new MenuBool("AssistManualWQ", "Infer and buffer Q after a manual W", true));
    HeadbuttMenu->Add(new MenuBool("PeelW", "Use full W to push a diver away", true));
    HeadbuttMenu->Add(new MenuSlider("PeelMinimumGain", "Minimum peel separation gain", 170, 60, 500));
    HeadbuttMenu->Add(new MenuBool("UseWallPin", "Preserve full W against thick terrain", true));
    HeadbuttMenu->Add(new MenuSlider("WallMinTravel", "Minimum useful wall-push distance", 90, 20, 300));
    HeadbuttMenu->Add(new MenuSlider("WallMaxTravel", "Maximum wall-push distance considered a pin", 520, 180, 700));
    HeadbuttMenu->Add(new MenuSlider("WallQDelay", "Q start delay after wall contact (ms)", 470, 180, 650));
    HeadbuttMenu->Add(new MenuBool("UseInsec", "Q, reposition, then W toward allies/turret", true));
    HeadbuttMenu->Add(new MenuSlider("InsecMinimumGain", "Minimum W distance gained toward team", 260, 100, 600));
    HeadbuttMenu->Add(new MenuSlider("InsecStandDistance", "Behind-target coaching distance", 175, 120, 280));
    HeadbuttMenu->Add(new MenuSlider("InsecWalkDelay", "Earliest W after Q for reposition (ms)", 360, 180, 700));
    HeadbuttMenu->Add(new MenuSlider("HarassWMinimumGain", "Harass full-W minimum teamward gain", 380, 150, 650));
    HeadbuttMenu->Add(new MenuBool("InterruptW", "Interrupt out-of-Q channels with full W", true));
    HeadbuttMenu->Add(new MenuBool("EscapeW", "Headbutt a unit toward the flee cursor", true));
    HeadbuttMenu->Add(new MenuSlider("EscapeMinimumGain", "Minimum escape W route gain", 130, 30, 500));
    HeadbuttMenu->Add(new MenuBool("RespectDashHazards", "Avoid Poppy/Taliyah/Cassiopeia dash zones", true));

    PulverizeMenu = TacticsMenu->AddSubMenu(new Menu(
        "Pulverize", "Pulverize timing and reactions"));
    PulverizeMenu->Add(new MenuBool("PeelQ", "Q a diver before spending Headbutt", true));
    PulverizeMenu->Add(new MenuBool("InterruptQ", "Interrupt channels inside Q radius", true));
    PulverizeMenu->Add(new MenuBool("FleeQ", "Q close pursuers during explicit flee", true));
    PulverizeMenu->Add(new MenuSlider("AoeEnemies", "Minimum enemies for proactive AoE Q", 2, 1, 5));
    PulverizeMenu->Add(new MenuSeparator(
        "BufferRule", "Buffered Q is immediate; wall, peel and insec branches deliberately keep full W."));

    TrampleMenu = TacticsMenu->AddSubMenu(new Menu(
        "Trample", "Trample pulse and empowered-AA ownership"));
    TrampleMenu->Add(new MenuBool("FourStackAttack", "Start AA at 4 stacks when pulse 5 lands before impact", true));
    TrampleMenu->Add(new MenuBool("ForceBestStunTarget", "Force orbwalker only to the best E-stun champion", true));
    TrampleMenu->Add(new MenuBool("HoldForPriorityChampion", "Block a lower-priority champion while E stun is ready", true));
    TrampleMenu->Add(new MenuSeparator(
        "NoMinionWaste", "At 4/5 stacks, minion attacks are blocked so the one-second stun is not wasted."));
    ...
    UltimateMenu->Add(new MenuBool("CleanseHardCC", "R meaningful hard CC under danger", true));
    UltimateMenu->Add(new MenuSlider("MinimumCCMs", "Minimum stun/root/silence duration (ms)", 650, 250, 1400));
    UltimateMenu->Add(new MenuSlider("CleanseHp", "Hard-CC cleanse danger HP (%)", 58, 20, 90));
    UltimateMenu->Add(new MenuBool("CleanseAirborneInLethalDanger", "R airborne only under lethal/turret danger", true));
    UltimateMenu->Add(new MenuSlider("BurstPercent", "R when recent burst is this % of current HP", 42, 15, 90));
    UltimateMenu->Add(new MenuSlider("TankMinimumEnemies", "Enemies required for proactive tank R", 3, 1, 5));
    UltimateMenu->Add(new MenuSlider("TankHp", "Multi-enemy tank R HP (%)", 56, 20, 90));
    UltimateMenu->Add(new MenuBool("UseAfterTurretAggro", "Delay dive R until turret aggro is real", true));
    UltimateMenu->Add(new MenuSlider("TurretShotsBeforeR", "Turret attacks observed before R", 1, 1, 3));
    ...
    UltimateMenu->Add(new MenuSeparator(
        "TrueDamage", "R reduces physical/magic damage; true damage is never counted as mitigated."));

    FarmMenu = TacticsMenu->AddSubMenu(new Menu(
        "Farm", "Support-safe farming"));
    FarmMenu->Add(new MenuBool("JungleAbilities", "Allow abilities on jungle camps", false));
    ...
    FarmMenu->Add(new MenuSeparator(
        "NoLaneFarm", "Q/W/E never take lane minions automatically."));

    CoachMenu = TacticsMenu->AddSubMenu(new Menu(
        "Coach", "One-trick visual coaching"));
    CoachMenu->Add(new MenuBool("DrawQ", "Draw real Pulverize effect radius", true));
    CoachMenu->Add(new MenuBool("DrawKnockback", "Draw planned W landing and wall stop", true));
    CoachMenu->Add(new MenuBool("DrawInsec", "Draw player reposition point and W goal", true));
    CoachMenu->Add(new MenuBool("DrawPeel", "Mark protected ally and current diver", true));
    CoachMenu->Add(new MenuBool("DrawEStun", "Mark the reserved empowered-AA target", true));
    CoachMenu->Add(new MenuBool("DrawState", "Draw posture, W branch, E/P stacks and R state", true));
}
