...
inline SDK::HitChance RequiredQHitchance(QPurpose purpose,
    ...
    case 2: return SDK::HitChance::VeryHigh;
    case 3: return SDK::HitChance::Immobile;
    default: return SDK::HitChance::High;
    }
}
...
inline void BuildMenu(Menu* root) {
    ...
    TacticsMenu->Add(new MenuBool(
        "KillSecure", "Secure with exact mitigated Q damage", true));
    TacticsMenu->Add(new MenuSlider(
        "DefensiveHp", "Defensive posture health threshold (%)",
        42, 10, 90));
    TacticsMenu->Add(new MenuSlider(
    ...
    TacticsMenu->Add(new MenuSeparator(
        "Ownership",
        "Movement, attacks, Flash, portal entry and chime collection remain player-owned."));

    RoleMenu = TacticsMenu->AddSubMenu(new Menu(
        "Conductor", "Carry protection and catch posture"));
    RoleMenu->Add(new MenuBool(
        "RespectCursor", "Require cursor agreement for automatic catches", true));
    RoleMenu->Add(new MenuSeparator(
        "Carry",
        "The protected ally is re-ranked from damage, range, health and live enemy pressure."));

    PassiveMenu = TacticsMenu->AddSubMenu(new Menu(
        "TravelersCall", "Chime and meep route policy"));
    PassiveMenu->Add(new MenuBool(
        "DrawChimeRoute", "Suggest chimes that fit the player's current route", true));
    PassiveMenu->Add(new MenuSlider(
        "RouteScore", "Minimum safe chime route score", 180, -300, 500));
    PassiveMenu->Add(new MenuSeparator(
        "NoMovement",
        "Chimes are advisory only; Bard never abandons the carry or redirects movement."));
    ...
    QMenu->Add(new MenuBool(
        "StasisExitQ", "Time Q for the exact end of Bard R stasis", true));
    QMenu->Add(new MenuSlider(
        "StasisEarlyMs", "R-exit Q early bias (ms)", 45, 0, 140));
    QMenu->Add(new MenuBool(
        "PortalExitQ", "Time Q at an enemy exiting Bard's portal", true));
    QMenu->Add(new MenuSlider(
        "PortalEarlyMs", "Portal-exit Q early bias (ms)", 55, 0, 160));
    QMenu->Add(new MenuBool(
        "FleeQ", "Slow or stun the closest pursuer while fleeing", true));
    QMenu->Add(new MenuSeparator(
        "MeepFirst",
        "In attack range Bard waits for meep-AA before ordinary Q; immediate stuns, interrupts and lethal Q bypass the wait."));

    WMenu = TacticsMenu->AddSubMenu(new Menu(
        "CaretakersShrine", "Direct heal, speed and shrine economy"));
    WMenu->Add(new MenuBool(
        "EmergencyDirect", "Direct-cast W on a threatened ally", true));
    WMenu->Add(new MenuSlider(
        "EmergencyHp", "Emergency ally health threshold (%)", 34, 5, 80));
    WMenu->Add(new MenuSlider(
        "EmergencyScore", "Minimum incoming-threat heal score", 520, 100, 1400));
    WMenu->Add(new MenuBool(
        "CombatSpeed", "Use direct W for a valuable combat speed swing", true));
    WMenu->Add(new MenuBool(
        "RetreatSpeed", "Use direct W on the retreating carry/Bard", true));
    WMenu->Add(new MenuBool(
        "GroundShrines", "Prepare spaced shrines before objectives/lane guard", true));
    WMenu->Add(new MenuBool(
        "ReserveLast", "Keep the final W charge outside emergencies", true));

    EMenu = TacticsMenu->AddSubMenu(new Menu(
        "MagicalJourney", "Terrain trace and player-led portal"));
    EMenu->Add(new MenuBool(
        "DefensiveEscape", "Create a safe portal against committed lethal pressure", true));
    EMenu->Add(new MenuBool(
        "FleePortal", "Create a cursor-aligned escape portal in flee mode", true));
    EMenu->Add(new MenuBool(
        "RespectDashHazards", "Reject exits into anti-dash zones", true));
    ...
    EMenu->Add(new MenuSeparator(
        "PlayerEntry",
        "The controller creates a verified one-way portal but never clicks or walks into it."));

    RMenu = TacticsMenu->AddSubMenu(new Menu(
        "TemperedFate", "Mixed-team stasis and no-grief policy"));
    RMenu->Add(new MenuBool(
        "AutomaticCatch", "Catch an overextended target within safe R range", true));
    RMenu->Add(new MenuSlider(
        "MaximumCatchRange", "Maximum automatic catch range", 1650, 700, 2800));
    RMenu->Add(new MenuSlider(
        "CatchFollowup", "Allies required to follow an automatic catch", 1, 0, 4));
    RMenu->Add(new MenuBool(
        "BacklineIsolation", "Stasis an unfocused backline pocket", true));
    RMenu->Add(new MenuSlider(
        "IsolationRange", "Maximum isolation R range", 1850, 800, 2800));
    RMenu->Add(new MenuBool(
        "SaveAlly", "Stasis an ally only against tracked lethal pressure", true));
    RMenu->Add(new MenuSlider(
        "SaveAllyHp", "Targeted ally save threshold (%)", 28, 5, 65));
    RMenu->Add(new MenuBool(
        "Interrupt", "Use R only when Q cannot interrupt in time", true));
    RMenu->Add(new MenuSlider(
        "FleeMinimum", "Pursuers required for flee R", 2, 1, 5));
    RMenu->Add(new MenuBool(
        "FleeR", "Use R as the final multi-pursuer disengage", true));
    RMenu->Add(new MenuBool(
        "AssistManualR", "Assist a player-cast R with exact exit Q", true));
    ...
    RMenu->Add(new MenuSeparator(
        "NoGrief",
        "Automatic R rejects allied channels, allies already fighting, secured objectives and focused low-health enemies."));

    FarmMenu = TacticsMenu->AddSubMenu(new Menu(
        "Farm", "Exact two-body Q farming only"));
    FarmMenu->Add(new MenuBool(
        "UseQ", "Use Q only for ordered double hits or exact last hits", true));
    FarmMenu->Add(new MenuBool(
        "HoldForChampion", "Hold farming Q while a champion contests", true));
    ...
    FarmMenu->Add(new MenuSeparator(
        "NoWaste",
        "W, E and R are never farming tools; Orbwalker owns attacks and movement."));
    ...
    CoachMenu->Add(new MenuBool(
        "DrawQ", "Draw Q first contact and second shackle", true));
    CoachMenu->Add(new MenuBool(
        "DrawShrines", "Draw shrine charge state", true));
    CoachMenu->Add(new MenuBool(
        "DrawPortal", "Draw selected portal entrance and exit", true));
    CoachMenu->Add(new MenuBool(
        "DrawR", "Draw the last evaluated R mixed-team circle", true));
    CoachMenu->Add(new MenuBool(
        "DrawProtectedAlly", "Mark the dynamically protected carry", true));
    CoachMenu->Add(new MenuBool(
        "DrawState", "Draw posture, sequence, chimes and ownership", true));
}
...
inline constexpr const char* Scenarios[] = {
    ...
    "Reject exits into anti-dash zones",
