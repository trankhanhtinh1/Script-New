...
inline void BuildMenu(Menu* root) {
    ...
    TacticsMenu->Add(new MenuBool(
        "KillSecure", "Use exact Q/W/E/R lethal windows", true));
    TacticsMenu->Add(new MenuSlider(
        "KiteHp", "Kite posture health threshold (%)", 36, 10, 80));
    TacticsMenu->Add(new MenuSlider(
        "ManualOwnershipMs", "Yield after player spell input (ms)",
        520, 180, 1200));
    TacticsMenu->Add(new MenuSeparator(
        "SecondEntry",
        "Bel'Veth waits for enemy commitment in multi-enemy fights; attacks, movement, Flash and Smite remain player-owned."));
    ...
    PassiveMenu->Add(new MenuBool(
        "ProtectSameTarget",
        "Reject an orbwalker minion swap that would break selected champion R ramp",
        true));
    PassiveMenu->Add(new MenuSeparator(
        "Weave",
        "Every spell grants two temporary attacks; controller preserves AA-Q-AA without issuing attacks itself."));

    QMenu = TacticsMenu->AddSubMenu(new Menu(
        "VoidSurge", "Four map-fixed Q sectors and AA reset"));
    QMenu->Add(new MenuBool(
        "Combo", "Use conserved Q sectors in combo", true));
    QMenu->Add(new MenuBool(
        "AutoReset", "Use Q immediately after a confirmed attack", true));
    QMenu->Add(new MenuBool(
        "Harass", "Use only AA-Q harass windows", true));
    QMenu->Add(new MenuBool(
        "Flee", "Use cursor-aligned Q sectors while fleeing", true));
    QMenu->Add(new MenuBool(
        "EvadeSkillshots", "Spend a safe sector on a tracked line skillshot", true));
    QMenu->Add(new MenuBool(
        "WallFlanks", "Use true-form Q only through traced terrain with an exit", true));
    QMenu->Add(new MenuBool(
        "AllowLethalDive", "Allow a lethal Q endpoint under enemy turret", false));
    QMenu->Add(new MenuSlider(
        "MaxEndpointEnemies", "Maximum enemies at ordinary Q endpoint", 2, 1, 5));
    QMenu->Add(new MenuSeparator(
        "Calibration",
        "BelvethQHudIcon0..15 is learned from observed casts; no patch-fragile bit order is hard-coded."));

    WMenu = TacticsMenu->AddSubMenu(new Menu(
        "AboveAndBelow", "Reliable knock-up and multi-sector Q refund"));
    WMenu->Add(new MenuBool(
        "Combo", "Use W for a reliable lock or spent-Q refund", true));
    WMenu->Add(new MenuBool(
        "Harass", "Use W in harass only when it refunds Q", true));
    WMenu->Add(new MenuBool(
        "Flee", "Use W to peel a committed pursuer", true));
    WMenu->Add(new MenuList(
        "Hitchance", "Ordinary W prediction",
        { "Medium", "High", "Very high", "Immobile only" }, 1));
    WMenu->Add(new MenuSeparator(
        "NoRawW",
        "Mobile targets must first dash, slow, commit or become crowd-controlled; multi-hit W can refresh multiple quadrants."));

    EMenu = TacticsMenu->AddSubMenu(new Menu(
        "RoyalMaelstrom", "Forced-target execute and damage reduction"));
    EMenu->Add(new MenuBool(
        "Combo", "Use E late after a secured W/Q window", true));
    EMenu->Add(new MenuBool(
        "Defensive", "Use E against meaningful reducible incoming burst", true));
    EMenu->Add(new MenuBool(
        "Jungle", "Use E for jungle sustain and epic secure", true));
    EMenu->Add(new MenuSlider(
        "ExecuteHp", "Champion E execute threshold (%)", 34, 10, 60));
    EMenu->Add(new MenuSeparator(
        "ForcedTarget",
        "E is authorized only after selecting the real lowest-current-health-percent unit in 500 range; it is held against an unsecured runner."));
    EMenu->Add(new MenuSeparator(
        "Cancel",
        "After 0.75s, E cancels only for a lethal Q dodge, a safe chase after target escape, or explicit cursor retreat after danger ends."));

    RMenu = TacticsMenu->AddSubMenu(new Menu(
        "EndlessBanquet", "Global coral consumption and form economy"));
    RMenu->Add(new MenuBool(
        "Automatic", "Consume a safe valuable coral automatically", true));
    RMenu->Add(new MenuBool(
        "Survival", "Use coral heal against tracked lethal reducible pressure", true));
    RMenu->Add(new MenuSlider(
        "HealHp", "Coral heal health threshold (%)", 36, 5, 75));
    ...
    RMenu->Add(new MenuSeparator(
        "GlobalConsume",
        "Casting R consumes every existing coral. Normal and enhanced form duration, expiry, heal, true-damage AoE and endpoint safety are valued together."));

    FarmMenu = TacticsMenu->AddSubMenu(new Menu(
        "Farm", "AA-reset clear and jungle wall cancel"));
    FarmMenu->Add(new MenuBool(
        "LaneQ", "Use lane Q only after AA or for exact last hit", true));
    FarmMenu->Add(new MenuBool(
        "LaneW", "Use lane W only for at least three hits", false));
    FarmMenu->Add(new MenuBool(
        "JungleQ", "Use jungle AA-Q and wall-shortened Q", true));
    FarmMenu->Add(new MenuBool(
        "JungleW", "Use jungle W for at least two monsters", true));
    FarmMenu->Add(new MenuSlider(
        "ReserveQ", "Q sectors reserved while farming", 1, 0, 3));
    FarmMenu->Add(new MenuSeparator(
        "PlayerOwned",
        "Orbwalker owns attacks and movement; Flash and Smite are never automated."));

    CoachMenu = TacticsMenu->AddSubMenu(new Menu(
        "Coach", "Bel'Veth one-trick geometry and state"));
    CoachMenu->Add(new MenuBool(
        "DrawQ", "Draw every independently ready Q sector", true));
    CoachMenu->Add(new MenuBool(
        "DrawW", "Draw the last selected W line", true));
    CoachMenu->Add(new MenuBool(
        "DrawE", "Draw E radius and real forced target", true));
    CoachMenu->Add(new MenuBool(
        "DrawCorals", "Draw ordinary and enhanced coral payoff zones", true));
    CoachMenu->Add(new MenuBool(
        "DrawState", "Draw posture, sequence, Q/passive/form state", true));
}
