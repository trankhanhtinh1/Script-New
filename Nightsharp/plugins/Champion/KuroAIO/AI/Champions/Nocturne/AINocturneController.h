...
inline bool CastFleeQ() {
    ...
    if (endpoint.IsZero() || SDK::NavMesh::IsWall(endpoint) ||
        ProjectileWallBlocksFromPlayer(endpoint, kQMissileRadius)) return false;
    if (!Engine::ControllerCastPosition(0, endpoint)) return false;
    QCastTick = Now();
    QTargetId = 0;
    QTrailStart = player.Position();
...
inline bool TryFarmQ(Mode mode) {
    ...
    if (bestHits < minimum || best.IsZero() ||
        ProjectileWallBlocksFromPlayer(best, kQMissileRadius) ||
...
inline constexpr ChampionController Controller = [] {
    ...
    controller.ChampionId = SDK::ChampionId::Nocturne;
