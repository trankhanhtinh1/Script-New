...
inline bool CastHumanQ(const AIHeroClient& target, Mode mode, bool reactive = false) {
    ...
    if (!aim.IsValid() || aim.IsZero() || player.Position().Distance2D(aim) >
        ...
        SDK::NavMesh::IsWallBetween(player.Position(), aim, kJavelinWidth * 0.5f) ||
        ProjectileWallBlocksFromPlayer(aim, kJavelinWidth * 0.5f)) return false;
...
inline constexpr ChampionController Controller = [] {
    ...
    controller.ChampionId = SDK::ChampionId::Nidalee;
