...
inline bool MordekaiserCanPunishShroud() {
    ...
    for (const auto& enemy : GameObjects::EnemyHeroes()) {
        if (ChampionIs(enemy, SDK::ChampionId::Mordekaiser) &&
...
inline bool HasReliableRevealThreat() {
    ...
    for (const auto& enemy : GameObjects::EnemyHeroes()) {
        ...
        if ((ChampionIs(enemy, SDK::ChampionId::LeeSin) && distance <= 1100.0f &&
             EnemySpellReady(enemy, SDK::SpellSlot::Q)) ||
            (ChampionIs(enemy, SDK::ChampionId::Lulu) && distance <= 650.0f &&
             EnemySpellReady(enemy, SDK::SpellSlot::E)) ||
            (ChampionIs(enemy, SDK::ChampionId::Karma) && distance <= 700.0f &&
             EnemySpellReady(enemy, SDK::SpellSlot::W)) ||
            (ChampionIs(enemy, SDK::ChampionId::Rengar) && EnemySpellReady(enemy, SDK::SpellSlot::R))) {
            return true;
        }
    }
    return false;
...
inline bool CastE1Raw(const Vector3& castPosition,
    ...
    if (ControllerHelpers::ProjectileWallBlocksFromPlayer(castPosition, 60.0f)) {
        return false;
    }
...
inline constexpr ChampionController Controller = [] {
    ...
    controller.ChampionId = SDK::ChampionId::Akali;
