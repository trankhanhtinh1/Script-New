...
inline bool CastQ(const AIHeroClient& target,
    ...
    if (mode == Mode::Combo && !guaranteed && !killSecure) {
        needed = SDK::HitChance::Medium;
    }
    ...
    if (ControllerHelpers::ProjectileWallBlocksFromPlayer(castPosition, 50.0f)) {
        return false;
    }
...
inline void BuildMenu(Menu* root) {
    ...
    OrbMenu->Add(new MenuBool("TipDoubleHit", "Q tip double-hit aim", true));
    OrbMenu->Add(new MenuSlider("TipConfidence", "Min tip double-hit (%)", 62, 35, 90));
    OrbMenu->Add(new MenuBool("RushRedirect", "R only to fix return-Q miss", true));
    OrbMenu->Add(new MenuSlider("KeepCurrentHit", "Skip R if return-Q >= (%)", 58, 35, 90));
    OrbMenu->Add(new MenuSlider("RedirectHit", "Post-R return score (%)", 55, 35, 90));
    OrbMenu->Add(new MenuSlider("RedirectGain", "Min R return gain (%)", 22, 8, 60));

    CharmMenu = TacticsMenu->AddSubMenu(new Menu(
        "CharmWindows", "Charm certainty and teamfight role"));
    CharmMenu->Add(new MenuSlider("MinimumQuality", "Min Charm quality", 4, 2, 6));
    CharmMenu->Add(new MenuSlider("ReactiveQuality", "Min anti-dash quality", 2, 1, 5));
    CharmMenu->Add(new MenuSlider("RawMaxRange", "Max raw E range", 825, 550, 975));
    CharmMenu->Add(new MenuSlider("RushCharmRange", "Post-R E guarantee dist", 540, 325, 750));
    CharmMenu->Add(new MenuBool("HideEWithR", "E->R hides Charm anim", true));
    CharmMenu->Add(new MenuBool("HarassOnlyGuaranteed", "Harass E: CC/dash only", true));
    CharmMenu->Add(new MenuBool("PeelCarry", "Peel carry before dive", true));

    FoxFireMenu = TacticsMenu->AddSubMenu(new Menu(
        "FoxFire", "Fox-Fire target marking and movement"));
    FoxFireMenu->Add(new MenuBool("AAWAA", "AA->W->AA trade", true));
    FoxFireMenu->Add(new MenuBool("QWTrade", "Use quick Q -> W/AA", true));
    FoxFireMenu->Add(new MenuBool("DodgeWithW", "Use W move speed for a", true));
    FoxFireMenu->Add(new MenuSeparator(
        "WPriority", "W waits for Charm, the"));

    RushMenu = TacticsMenu->AddSubMenu(new Menu(
        "SpiritRush", "Spirit Rush charge economy"));
    RushMenu->Add(new MenuBool("UseToGuaranteeCharm", "R create a collision-free", true));
    RushMenu->Add(new MenuSlider("ReserveCharges", "Reserve R charges for exit", 1, 0, 2));
    RushMenu->Add(new MenuSlider("IsolatedTargetHp", "Commit isolated HP< (%)", 72, 25, 100));
    RushMenu->Add(new MenuSlider("EscapeHp", "Escape below HP (%)", 26, 10, 60));
    ...
    FarmMenu->Add(new MenuSlider("QMinions", "Min lane minions crossed by", 3, 1, 8));
    FarmMenu->Add(new MenuBool("LaneW", "W confirmed last hits", false));
    FarmMenu->Add(new MenuSlider("WLastHits", "Min W last hits", 2, 1, 3));
    FarmMenu->Add(new MenuBool("JungleW", "Use W on jungle monsters", true));
    FarmMenu->Add(new MenuSeparator(
        "NoFarmR", "R not used farming"));
...
inline constexpr ChampionController Controller = [] {
    ...
    controller.ChampionId = SDK::ChampionId::Ahri;
