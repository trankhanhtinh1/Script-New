...
inline void BuildMenu(Menu* root) {
    ...
    PassiveMenu->Add(new MenuSlider(
        "WeaveWindowMs", "Q weave after shot (ms)", 430, 180, 850));
    PassiveMenu->Add(new MenuSeparator(
        "ShotOwnership", "Champion damage/procs and"));
    ...
    QMenu->Add(new MenuBool(
        "HarassRequireReturn", "Require return hit harass", false));
    ...
    RogueMenu->Add(new MenuSlider(
        "OutOfCombatMs", "Required time out of combat", 2600, 1000, 6000));
    RogueMenu->Add(new MenuSlider(
        "ManaReservePercent", "Mana % kept after W", 55, 0, 100));
    RogueMenu->Add(new MenuSlider(
        "CleanupHP", "Scoundrel cleanup posture", 52, 10, 90));
    RogueMenu->Add(new MenuBool(
        "FleeW", "W in flee only after", true));
    ...
    SwingMenu->Add(new MenuSlider(
        "MaxSwingMs", "Max combat orbit (ms)", 2200, 650, 5000));
    SwingMenu->Add(new MenuSlider(
        "SwingTargetHP", "Aggressive E HP< (%)", 45, 10, 90));
    SwingMenu->Add(new MenuSlider(
        "ManaAfterE", "Mana retained after damage E", 40, 0, 220));
    SwingMenu->Add(new MenuSlider(
        "MinCommitHP", "Min HP nonlethal swing (%)", 34, 10, 80));
    SwingMenu->Add(new MenuSlider(
        "AssassinateHP", "Assassinate HP< (%)", 58, 15, 100));
    SwingMenu->Add(new MenuSlider(
        "EscapeHP", "Escape HP< (%)", 28, 10, 70));
    SwingMenu->Add(new MenuSlider(
        "EmergencyDismountHP", "Emergency E3 below own HP (%)", 24, 5, 65));
    SwingMenu->Add(new MenuSlider(
        "MaxCommitEnemies", "Max enemies nonlethal route", 2, 1, 5));
    SwingMenu->Add(new MenuBool(
        "RespectLockdown", "Reject landings into ready", true));
    SwingMenu->Add(new MenuBool(
        "AllowLethalDive", "Lethal turret dismount", false));
    ...
    UltimateMenu->Add(new MenuSlider(
        "EarlyReleaseMinBullets", "Min bullets for cover-aware", 2, 1, 7));
    UltimateMenu->Add(new MenuBool(
        "UseEForRBlocker", "E during R only to open the", true));
    UltimateMenu->Add(new MenuSlider(
        "RepositionLostBullets", "Reposition after this many", 2, 1, 6));
    UltimateMenu->Add(new MenuSlider(
...
inline constexpr ChampionController Controller = [] {
    ...
    controller.ChampionId = SDK::ChampionId::Akshan;
