...
inline FlashFrostPlan BuildQPlan(const AIHeroClient& target,
    ...
    if (Orbwalker::ActiveMode() == OrbwalkingMode::Combo && !reactive) {
        required = SDK::HitChance::High;
    }
...
inline void BuildMenu(Menu* root) {
    ...
    TacticsMenu->Add(new MenuBool(
        "PeelBeforeDamage", "Protect a pressured ally", true));
    ...
    FlashFrostMenu->Add(new MenuSlider(
        "HoldBeyond", "Hold Q beyond range", 720, 350, 1000));
    FlashFrostMenu->Add(new MenuSlider(
        "PassOvershoot", "Min Q overshoot before", 8, 0, 80));
    FlashFrostMenu->Add(new MenuSlider(
        "DetonateAoe", "Detonate Q min explosion", 2, 1, 5));
    FlashFrostMenu->Add(new MenuBool(
        "BreakSpellShield", "Use Q pass to consume an", true));
    FlashFrostMenu->Add(new MenuBool(
        "HoldForSecondE", "Keep Q for E2 setup", true));
    FlashFrostMenu->Add(new MenuBool(
        "ProtectDetonation", "Cancel only an AA that would", true));
    FlashFrostMenu->Add(new MenuBool(
        "Interrupt", "Q interrupt when impact", true));
    FlashFrostMenu->Add(new MenuBool(
        "AntiGapcloser", "Q vs committed gapcloser", true));
    FlashFrostMenu->Add(new MenuBool(
        "Peel", "Q peel protected ally", true));
    FlashFrostMenu->Add(new MenuBool(
        "Harass", "Use Q harass on", true));
    FlashFrostMenu->Add(new MenuSlider(
    ...
    WallMenu->Add(new MenuBool(
        "ForceQPath", "Use W during Q flight to", true));
    WallMenu->Add(new MenuBool(
        "CatchBeforeQ", "W first on target with a", true));
    WallMenu->Add(new MenuBool(
        "Peel", "Split diver from carry", true));
    WallMenu->Add(new MenuBool(
        "AntiGapcloser", "W across dash exit", true));
    WallMenu->Add(new MenuBool(
        "Interrupt", "W displace channels", true));
    WallMenu->Add(new MenuBool(
        "Flee", "Safe barrier on flee", true));
    ...
    FrostbiteMenu->Add(new MenuBool(
        "OrdinaryPoke", "Allow unempowered AA-E poke", false));
    FrostbiteMenu->Add(new MenuSlider(
        "OrdinaryPokeMana", "Mana E poke (%)", 78, 50, 100));
    ...
    StormMenu->Add(new MenuSlider(
        "MinimumScore", "Min R combo score", 360, 100, 1200));
    StormMenu->Add(new MenuSlider(
        "MaximumLead", "Max move lead R center", 190, 0, 300));
    StormMenu->Add(new MenuSlider(
        "FlatReserve", "Mana kept after R drain", 90, 0, 350));
    StormMenu->Add(new MenuSlider(
        "NoContactGrace", "R end grace (ms)", 520, 150, 1400));
    StormMenu->Add(new MenuBool(
        "Relocate", "End an empty mature R for a", true));
    StormMenu->Add(new MenuSlider(
    ...
    StormMenu->Add(new MenuBool(
        "AntiGapcloser", "R on committed dash", true));
    StormMenu->Add(new MenuBool(
        "Harass", "R harass (high mana)", false));
    StormMenu->Add(new MenuSlider(
    ...
    FarmMenu->Add(new MenuSlider(
        "QDetonateHits", "Min lane units Q", 2, 1, 8));
    FarmMenu->Add(new MenuBool(
        "UseE", "E for chilled jungle targets", true));
    FarmMenu->Add(new MenuBool(
        "LaneR", "Use R for a large lane wave", false));
    FarmMenu->Add(new MenuSlider(
        "RMinimumLane", "Minimum lane units for R", 5, 3, 10));
    FarmMenu->Add(new MenuBool(
        "JungleR", "R in multi-unit camps and on", true));
    FarmMenu->Add(new MenuSlider(
...
inline constexpr ChampionController Controller = [] {
    ...
    controller.ChampionId = SDK::ChampionId::Anivia;
