...
inline bool RHasChampionBlocker(const AIHeroClient& target) {
    ...
    Vector3 end = ControllerHelpers::PredictPosition(target, 0.50f);
    if (!end.IsValid() || end.IsZero()) end = target.Position();
...
inline bool CastR(const AIHeroClient& target, bool manual) {
    ...
    const Vector3 predictedTarget = ControllerHelpers::PredictPosition(target, 0.50f);
    context.ProjectileWall = RHasChampionBlocker(target) ||
        ProjectileWallBlocksFromPlayer(
            predictedTarget.IsValid() && !predictedTarget.IsZero()
                ? predictedTarget : target.Position(), 85.0f);
...
inline constexpr ChampionController Controller = [] {
    ...
    controller.ChampionId = SDK::ChampionId::Caitlyn;
