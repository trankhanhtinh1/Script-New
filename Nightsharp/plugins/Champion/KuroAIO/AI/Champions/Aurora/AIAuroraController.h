...
inline QPlan BuildQPlan(const AIBaseClient& primary,
    ...
    for (const float degrees : angles) {
        ...
        if (!reactive && !CursorDirectionAgrees(aim, -0.18f) &&
            Orbwalker::ActiveMode() != OrbwalkingMode::Combo) score -= 390.0f;
...
inline EPlan BuildEPlan(const AIBaseClient& primary,
    ...
    for (const float degrees : angles) {
        ...
        if (!reactive && !CursorDirectionAgrees(aim, -0.28f) &&
            purpose != EPurpose::RecoilPeel &&
            Orbwalker::ActiveMode() != OrbwalkingMode::Combo) score -= 190.0f;
        if (score > best.Score) {
            best.Aim = aim;
...
inline void BuildMenu(Menu* root) {
    ...
    TacticsMenu->Add(new MenuBool(
        "KillSecure", "Exact Q2/E/R kills", true));
    ...
    PassiveMenu->Add(new MenuBool(
        "SeekSecondProc", "Extend arena fights when a", true));
    ...
    HexMenu->Add(new MenuBool(
        "Harass", "Q before the player auto in", true));
    HexMenu->Add(new MenuSlider(
        "HarassMana", "Minimum mana for Q trades (%)", 48, 0, 100));
    HexMenu->Add(new MenuBool(
        "PreserveWindup", "Delay Q2 unless", true));
    HexMenu->Add(new MenuBool(
        "AlignMarkedWave", "Hold Q2 for the player's", true));
    ...
    VeilMenu->Add(new MenuBool(
        "OnlyAfterCCSpent", "Hold offensive W until key", true));
    VeilMenu->Add(new MenuBool(
        "Defensive", "W dodge after reset", true));
    VeilMenu->Add(new MenuBool(
        "Flee", "Use cursor-agreeing W wall", true));
    VeilMenu->Add(new MenuSlider(
        "MinimumRouteScore", "Min W route score", 160, -300, 1200));
    ...
    WeirdingMenu->Add(new MenuBool(
        "Harass", "E proc/poke", true));
    WeirdingMenu->Add(new MenuSlider(
        "HarassMana", "Min mana E poke (%)", 64, 0, 100));
    WeirdingMenu->Add(new MenuBool(
        "AntiGapcloser", "E diver with a safe recoil", true));
    WeirdingMenu->Add(new MenuBool(
        "BufferDisplacements", "Use E recoil to buffer a", true));
    WeirdingMenu->Add(new MenuBool(
        "Flee", "Slow pursuer, recoil", true));
    ...
    WorldsMenu->Add(new MenuBool(
        "OpenVerifiedAllIn", "Open R on allied CC or", true));
    WorldsMenu->Add(new MenuSlider(
        "MinimumHits", "Minimum offensive R hits", 2, 1, 5));
    WorldsMenu->Add(new MenuBool(
        "BufferOneInstanceCC", "Use unstoppable R leap vs", true));
    WorldsMenu->Add(new MenuBool(
        "SelfPeel", "Use R defensively when W/E", true));
    WorldsMenu->Add(new MenuBool(
        "EPortalBuffer", "E inward so recoil crosses a", true));
    WorldsMenu->Add(new MenuBool(
        "WHiddenPortal", "Allow W portal bounce", false));
    WorldsMenu->Add(new MenuBool(
        "KillSecure", "Exact mitigated R damage for", true));
    WorldsMenu->Add(new MenuBool(
    ...
    FarmMenu->Add(new MenuBool(
        "UseE", "E only after marked-wave", true));
    FarmMenu->Add(new MenuSlider(
...
inline constexpr ChampionController Controller = [] {
    ...
    controller.ChampionId = SDK::ChampionId::Aurora;
