 ...
 *
 * SDK plugins (Orbwalker, Target Selector) are auto-loaded by SDK::Bootstrap
 * and appear as primary sidebar categories below Core (no manager UI).
 */
...
namespace NightSharpMenu {
    ...
    inline int  pluginManagerFilter = 0;    // 0 All, 1 Champion, 2 Utility, 3 Core, 4 Misc
    ...
    inline const SidebarEntry CORE_SECONDARY[CORE_SECONDARY_COUNT] = {
        { "Language" }, { "Menu" }, { "Debug Info" }, { "Plugins" },
    };
    ...
    inline bool PluginManagerRowVisible(int idx, PluginRegistry::PluginKind kind) {
        ...
        if (p.Kind != kind || !p.Name) return false;
        ...
        const auto category = FilterCategoryFromIndex(pluginManagerFilter);
    ...
    inline int CountPluginManagerRows(PluginRegistry::PluginKind kind) {
        ...
        for (int i = 0; i < PluginRegistry::PluginCount; i++) {
            if (PluginManagerRowVisible(i, kind)) count++;
        }
        return count;
    }
    ...
    inline void DrawPluginManagerRows(PluginRegistry::PluginKind kind, const char* emptyText, int idBase = 0) {
        int pluginCount = CountPluginManagerRows(kind);
        if (pluginCount == 0) {
        ...
        for (int i = 0; i < PluginRegistry::PluginCount; i++) {
            ...
            if (!PluginManagerRowVisible(i, kind)) continue;
            ...
            ImGui::SameLine(0, 8);
            ImGui::TextColored(ImVec4(0.48f,0.62f,0.78f,0.9f), "[%s]", PluginRegistry::CategoryName(p.Category));

            if (p.Category == PluginRegistry::PluginCategory::Champion && p.ChampionName && p.ChampionName[0]) {
                ImGui::SameLine(0, 6);
                ImGui::TextColored(ImVec4(0.68f,0.78f,1.0f,0.9f), "%s", p.ChampionName);
            }

            if (canLoad && !p.MenuRoot) {
                ImGui::SameLine(0, 8);
    ...
    inline void DrawPluginsSection() {
        ...
        int extCount = CountPluginManagerRows(PluginRegistry::PluginKind::External);
        if (extCount == 0) {
            ImGui::Spacing();
            ImGui::TextColored(ImVec4(0.5f,0.5f,0.6f,1.0f), "No external plugins for this filter.");
            return;
        }
    ...
    inline void DrawCoreContentPanel(int secondaryIdx) {
        ...
        else if (secondaryIdx == 3) DrawPluginsSection();
