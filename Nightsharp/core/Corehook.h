...
inline int InstallInlineHooks() {
    ...
    // === Stub Hardening: Scan inline syscall ===
    // NOP inline syscall shellcode (NtTerminateProcess) trong stub.dll
    // Chay SAU CRCBypass::Install (shadow copy da co ban sach).
    if (NIGHTSHARP_ENABLE_STUB_HARDENING) {
        (void)StubHardening::ScanInlineSyscalls();
    }
...
inline bool InstallHook(HookId id) {
    ...
    if (NIGHTSHARP_ENABLE_CRC_BYPASS) {
        ...
        if (InterlockedCompareExchange(&s_crcInstalled, 1, 0) == 0) {
            ...
            if (CRCBypass::g_crcJneAddr) {
                uint8_t check[6] = {};
                __try {
                    std::memcpy(check, (void*)CRCBypass::g_crcJneAddr, 6);
                } __except (EXCEPTION_EXECUTE_HANDLER) {}
                Logf("CRC Bypass: JNE @ 0x%llX bytes=%02X %02X %02X %02X %02X %02X",
                     (unsigned long long)CRCBypass::g_crcJneAddr,
