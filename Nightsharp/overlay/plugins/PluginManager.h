...
#include "../sdk/Core/ObjectCachePreWarm.h"
...
namespace Plugins {
    ...
    public:
        ...
        // Helper: runs one plugin's OnUpdate inside an SEH frame.
        // Must be in a separate function — MSVC C2712 forbids __try in functions
        // that have C++ object unwinding (RAII locals from NS_PROFILE etc.).
        void RunPluginUpdate(IPlugin* plugin) {
            __try {
                plugin->OnUpdate();
            }
            __except (NightSharpDebug::CrashReporter::LogAndDumpException(
        ...
        void OnUpdate() {
            NS_PROFILE("PluginManager::OnUpdate");

            // Pre-warm ObjectCache AiSnap entries for all objects the orbwalker
            // and plugins will read this frame. Moves cold-miss cost from the hot
            // orbwalk loop into one tight pass with predictable memory access.
            ObjectCache::PreWarm();
