...
namespace ObjectCache {
    ...
    struct SnapshotEntry {
        ...
        bool hasAiSnap = false;
        ::CoreAiManager::Snapshot aiSnap;
    ...
    // GetAiSnap: lazily populates CoreAiManager::Snapshot for `a` once per phase.
    // ReadSnapshot (after Phase 1 fix) calls Get() exactly once, so all AI fields
    // (isMoving, isDashing, velocity, serverPosition, waypoint metadata) come from
    // a single native manager resolve.
    inline const ::CoreAiManager::Snapshot& GetAiSnap(uintptr_t a) {
        SnapshotEntry* e = GetEntry(a);
        if (!e->hasAiSnap) {
            e->aiSnap = ::CoreAiManager::ReadSnapshot(a);
            e->hasAiSnap = true;
        }
        return e->aiSnap;
    }
    // Waypoint cache — reuses aiSnap.manager/navArray/segment data so the
    // ManagerRef from ReadSnapshot is not re-resolved a second time.
    inline std::vector<Vector3>& GetWaypoints(uintptr_t a, int maxPoints) {
        ...
        if (!e->hasWaypoints) {
            ...
            // Ensure aiSnap is populated so we can use its pre-read navArray/segment data.
            if (!e->hasAiSnap) {
                e->aiSnap = ::CoreAiManager::ReadSnapshot(a);
                e->hasAiSnap = true;
            }
            const auto& ai = e->aiSnap;
            if (ai.manager && ::CoreAiManager::IsSaneWaypointCount(ai.segmentsCount)) {
                const ::CoreAiManager::ManagerRef mgr{ai.manager};
                Vec3 points[::CoreAiManager::kMaxWaypoints] = {};
                const int count = ::CoreAiManager::CopyWaypointsFromRef(mgr, points, maxPoints);
    ...
    // ServerPosition served from the unified AiSnap — eliminates the old separate
    // hasServerPosition/serverPosition lazy-init path that did its own Get() call.
    inline Vector3 GetServerPosition(uintptr_t a) {
        return GetAiSnap(a).serverPosition;
    }
}
...
public:
    ...
    ::CoreAiManager::Snapshot AiManagerSnapshot() const {
        // Served from the per-phase AiSnap cache — no new native manager resolve.
        return ObjectCache::GetAiSnap(Address());
    }
    ...
    Vector3 PreviousPosition() const {
        const auto& ai = ObjectCache::GetAiSnap(Address());
        return ::CoreAiManager::IsUsablePosition(ai.startPath) ? ai.startPath : ai.serverPosition;
    }

    Vector3 Direction() const {
        const auto& ai = ObjectCache::GetAiSnap(Address());
        Vec3 dir = ::CoreAiManager::Normalized2D(ai.velocity);
        if (!dir.IsZero()) return dir;
        if (::CoreAiManager::IsUsablePosition(ai.serverPosition) &&
            ::CoreAiManager::IsUsablePosition(ai.targetPosition) &&
            ai.serverPosition.Distance2D(ai.targetPosition) > 1.0f) {
            return ::CoreAiManager::Normalized2D(ai.targetPosition - ai.serverPosition);
        }
        return {};
    }

    Vector3 Velocity() const {
        return ObjectCache::GetAiSnap(Address()).velocity;
    }

    Vector3 PathStart() const {
        return ObjectCache::GetAiSnap(Address()).startPath;
    }

    Vector3 PathEnd() const {
        const auto& ai = ObjectCache::GetAiSnap(Address());
        return ::CoreAiManager::IsUsablePosition(ai.targetPosition) ? ai.targetPosition : Vec3{};
    }

    Vector3 OrderPosition() const {
        return ObjectCache::GetAiSnap(Address()).orderPosition;
    }

    bool HasPath() const {
        const auto& ai = ObjectCache::GetAiSnap(Address());
        if (::CoreAiManager::IsSaneWaypointCount(ai.segmentsCount) &&
            ai.currentSegment < ai.segmentsCount) return true;
        return ::CoreAiManager::IsUsablePosition(ai.serverPosition) &&
               ::CoreAiManager::IsUsablePosition(ai.targetPosition) &&
               ai.serverPosition.Distance2D(ai.targetPosition) > 5.0f;
    }

    bool IsMoving() const {
        return ObjectCache::GetAiSnap(Address()).isMoving;
    }

    bool IsDashing() const {
        return ObjectCache::GetAiSnap(Address()).isDashing;
    }

    bool HasArrived() const {
        const auto& ai = ObjectCache::GetAiSnap(Address());
        if (ai.isMoving || HasPath()) return false;
        return ai.targetPosition.IsZero() ||
               (::CoreAiManager::IsUsablePosition(ai.serverPosition) &&
                ai.serverPosition.Distance2D(ai.targetPosition) <= 5.0f);
    }

    int CurrentPathSegment() const {
        return ObjectCache::GetAiSnap(Address()).currentSegment;
    }

    int WaypointCount() const {
        const auto& ai = ObjectCache::GetAiSnap(Address());
        if (!::CoreAiManager::IsSaneWaypointCount(ai.segmentsCount)) return 0;
        const int current = std::clamp(ai.currentSegment, 0, ai.segmentsCount);
        return std::max(0, ai.segmentsCount - current);
    }

    float DashSpeed() const {
        return ObjectCache::GetAiSnap(Address()).dashSpeed;
    }

    float DashDistanceRemaining() const {
        const auto& ai = ObjectCache::GetAiSnap(Address());
        if (!ai.isDashing) return 0.0f;
        if (!::CoreAiManager::IsUsablePosition(ai.serverPosition) ||
            !::CoreAiManager::IsUsablePosition(ai.targetPosition)) return 0.0f;
        return ai.serverPosition.Distance2D(ai.targetPosition);
    }
