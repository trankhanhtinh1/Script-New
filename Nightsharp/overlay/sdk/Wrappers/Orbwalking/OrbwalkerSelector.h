...
private:
    ...
    std::vector<AIMinionClient> GetMinions(OrbwalkingMode mode) const {
        ...
        // Hoist player, position, and attack range once — used for BOTH the lane-minion
        // loop and the jungle loop to avoid two redundant player/range resolves (Bug 8).
...

        {
            ...
            for (const auto& minion : GetCachedEnemyMinions()) {
                ...
                // Pass pre-computed attackRange to avoid per-minion range re-resolve (Bug 5).
                if (!IsValidUnit(minion, attackRange)) continue;

                // Use const char* directly from snapshot — no std::string heap alloc (Bug 6).
                const char* baseName = minion.Snapshot().characterName;

                if (includeMinions && minion.IsMinion()) {
                    minionList.push_back(minion);
                } else if (attackSpecialMinions && IsSpecialMinion(baseName)) {
                    specialList.push_back(minion);
                } else if (attackClones && IsClone(baseName)) {
        ...
        if (includeMinions) {
            ...
            // Reuse playerPos and attackRange already computed above (Bug 8).
            for (const auto& j : GameObjects::Jungle()) {
                if (playerPos.Distance2D(j.Position()) > attackRange) continue;
                if (IsValidUnit(j, attackRange)) {
                    const char* jName = j.Snapshot().characterName;
                    if (_stricmp(jName, "gangplankbarrel") != 0) {
        ...
        if (MenuBool("attackBarrels")) {
            for (const auto& j : GameObjects::Jungle()) {
                if (IsValidUnit(j)) {
                    const char* jName = j.Snapshot().characterName;
                    if (_stricmp(jName, "gangplankbarrel") == 0 && j.Health() <= 1.0f) {
        ...
        finalList.erase(
            std::remove_if(finalList.begin(), finalList.end(),
                [](const AIMinionClient& m) {
                    return IsIgnoredMinion(m.Snapshot().characterName);
                }),
            finalList.end());
...
public:
    ...
    mutable uint32_t cachedEnemyMinionsGen_ = 0;

    const std::vector<AIMinionClient>& GetCachedEnemyMinions() const {
        const uint32_t curGen = CoreRuntime::g_ctx.phaseGeneration;
        if (cachedEnemyMinionsGen_ != curGen) {
    ...
    AttackableUnit GetTarget(OrbwalkingMode mode) {
            ...
            if (!turretMinions.empty()) {
                ...
                if (turretMinion != turretMinions.end()) {
                    ...
                    if (aggroTurret.IsValid()) {
                        ...
                        // Pass already-computed minions to ShouldWaitUnderTurret — avoids second scan.
        ...
        if (mode == OrbwalkingMode::LaneClear) {
            // Pass already-computed minions list — no second GetEnemyMinions() call (Bug 9 fix).
            if (!ShouldWait(minions)) {
    ...
    bool ShouldWait() const {
        return ShouldWait(GetEnemyMinions());
    }

    bool ShouldWait(const std::vector<AIMinionClient>& minions) const {
