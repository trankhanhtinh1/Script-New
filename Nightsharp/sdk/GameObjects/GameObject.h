...
namespace SDK {
	...
	public:
		...
		bool IsJungleMonster() const {
			...
			if (!IsNativeJungleMonster()) return false;
			...
			if (strstr(objectNamePtr, "SRU_Plant") != nullptr ||
				strstr(characterNamePtr, "SRU_Plant") != nullptr) {
				return false;
			}
		...
		bool IsNativeJungleMonster() const {
			typedef bool(__fastcall* Fn)(uintptr_t);
