...
namespace SDK {
    ...
    public:
        ...
        std::string GetName() const {
            ...
            // Legacy SDK layout.
            std::string name = ReadStringAt(address + Offset::SpellBook::DataSpellName);
            if (!name.empty()) {
                return name;
            }

            // Chimera reference layout: SpellData::SpellName = 0x28.
            name = ReadStringAt(address + 0x28);
            if (!name.empty()) {
                return name;
            }

            // Some call-sites may hand us a SpellInfo-like pointer instead of SpellData.
            uintptr_t maybeSpellData = Globals::Read<uintptr_t>(address + Offset::SpellBook::InfoSpellData);
            if (Globals::IsValidPtr(maybeSpellData) && maybeSpellData != address) {
                name = ReadStringAt(maybeSpellData + Offset::SpellBook::DataSpellName);
                if (!name.empty()) {
                    return name;
                }

                name = ReadStringAt(maybeSpellData + 0x28);
                if (!name.empty()) {
                    return name;
                }
            }
        ...
        // ====================================================================
    private:
        static std::string ReadStringAt(uintptr_t addr) {
            char buf[128] = {};
        ...
        uintptr_t GetResource() const {
            if (!IsValid()) return 0;
            uintptr_t res = Globals::Read<uintptr_t>(address + Offset::SpellBook::DataResourceBase);
            if (Globals::IsValidPtr(res)) {
                return res;
            }

            // Chimera reference layout: SpellDataResource = 0x50.
            res = Globals::Read<uintptr_t>(address + 0x50);
            if (Globals::IsValidPtr(res)) {
                return res;
            }

            // If this is actually a SpellInfo wrapper, try one extra dereference.
            uintptr_t maybeSpellData = Globals::Read<uintptr_t>(address + Offset::SpellBook::InfoSpellData);
            if (Globals::IsValidPtr(maybeSpellData)) {
                res = Globals::Read<uintptr_t>(maybeSpellData + Offset::SpellBook::DataResourceBase);
                if (Globals::IsValidPtr(res)) {
                    return res;
                }
                res = Globals::Read<uintptr_t>(maybeSpellData + 0x50);
                if (Globals::IsValidPtr(res)) {
                    return res;
                }
            }
    ...
    public:
        ...
        SpellData GetSpellData() const {
            ...
            // Some current builds store SpellData directly in the slot object.
            ptr = Globals::Read<uintptr_t>(address);
            if (Globals::IsValidPtr(ptr)) {
                SpellData direct(ptr);
                if (!direct.GetName().empty()) {
                    return direct;
                }
            }
