...
namespace SDK {
    ...
    public:
        ...
        static std::vector<GameObject> GetMinions() {
            ...
            uintptr_t list = Globals::Read<uintptr_t>(mgr + 0x8);
            int count = Globals::Read<int>(mgr + 0x10);
            // MinionManager can exceed 500 entries (lane + jungle + wards + helpers).
            // Keep a high cap to avoid dropping the whole list when count spikes.
            if (!Globals::IsValidPtr(list) || count <= 0 || count > 2000) return result;
